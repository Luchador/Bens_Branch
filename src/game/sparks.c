...
struct sparktype g_SparkTypes[] = {
	...
	//         |    |    length            |     |                                        decel
	//         |    |    |     width       |     |                                        |
	/*0x00*/ { 100, 28,  100,  1,   0,  0, 2,                60,  60,  15, 1, 0xfff0a0ff, 0xffffffff, 0.02 },
	...
	/*0x0f*/ { 70,  70,  250,  15,  0,  0, 6,                40,  10,  5,  0, 0x30304080, 0x30304080, 0.02 }, // Water
		...
		for (i = 0; i < ARRAYCOUNT(g_SparkGroups); i++) {
			...
			if (group->age != 0) {
				...
				if (render) {
					...
					mtxApplyAffineInPlace(camGetWorldToScreenMtxf(), &spd4);
