...
struct drawslot g_BgDrawSlots[61]; // 61 to 251
u8 *g_BgPrimaryData;
u32 var800a4920;
u32 g_BgSection3;
struct room *g_Rooms;
...
RoomNum g_GlareRooms[100]; // Tripled
...
s32 g_CamRoom = 1;
struct drawslot *g_BgSpecialDrawSlot = &g_BgDrawSlots[60]; // 60 to 250
...
void bgSetRoomOnscreen(s32 roomnum, s32 draworder, struct screenbox *box)
{
	...
	if ((g_Rooms[roomnum].flags & ROOMFLAG_DISABLEDBYSCRIPT) == 0) {
		...
		if (g_Rooms[roomnum].flags & ROOMFLAG_BBOXHACK) {
			box->xmin = g_BgDrawSlots[60].box.xmin; // 60 to 250
		...
		if (g_BgFrameCount == g_BgDrawSlotsByRoom[roomnum].updatedframe) {
			...
			if (g_BgNumAttemptedDrawSlots < 60) { // 60 to 250
