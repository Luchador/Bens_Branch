			...
			if (chr->onladder) {
				...
				if (chr->chrflags & CHRCFLAG_FORCETOGROUND) {
					...
					if (chr->fallspeed.y != 0.0f || chr->ground < chr->manground) {
						...
						integrateDampedMotion(&yincrement, &sp68, VAR(lvupdate60freal));
...
void chrInit(struct prop *prop, uint8_t *ailist)
{
	...
	chr->oldframe = 0;
	chr->magicframe = 0;
	//chr->magicspeed = VERSION >= VERSION_PAL_BETA ? 1 : 0.25; // Magic speed much slower on NTSC?
	chr->magicspeed = 0.25f;
		...
		struct coord *pos, RoomNum *rooms, float faceangle, uint8_t *ailist)
{
	...
	modelSetAnimPlaySpeed(model, g_ChrAnimSpeed, 0);
...
void chrHandleJointPositioned(int joint, Mtx *mtx)
{
	...
	if (g_CurModelChr->model->definition->skel == &g_SkelRobot) {
		...
		mtxApplyAffineTransformInPlace(camGetProjectionMtx(), (Mtx*)mtx);
		...
		if (joint == lshoulderjoint || joint == rshoulderjoint || joint == waistjoint || joint == neckjoint) {
			...
			if (xrot != 0.0f || yrot != 0.0f || zrot != 0.0f || scale != 1.0f) {
				...
				mtxApplyAffineTransformInPlace(camGetProjectionMtx(), mtx);
			...
			if (prop->flags & PROPFLAG_ONTHISSCREENTHISTICK) {
				...
				if (obj->model->definition->skel != &g_SkelWindowedDoor
					...
					if (!thing->unk00c) {
						...
						mtxInvertAffine((Mtx*)&thing->unk02c, (Mtx*)&thing->unk06c);
			...
			for (i = 0; i < var80062964; i++) {
				if (var80062960[i].unk004) {
					...
					if (thing->unk130 == 0) {
						...
						mtxApplyAffineTransform(camGetPlayerWorldToScreenMtx(), (Mtx*)&thing->unk02c, (Mtx*)&thing->unk0ec);
						mtx4TransformVec((Mtx*)&thing->unk0ec, &spa0, &sp70);
						camProjectViewToScreenAbsZ(&sp70, thing->unk134);
						mtx4TransformVec((Mtx*)&thing->unk0ec, &sp94, &sp70);
						camProjectViewToScreenAbsZ(&sp70, thing->unk13c);
						mtx4TransformVec((Mtx*)&thing->unk0ec, &sp88, &sp70);
...
Gfx *chrRender(struct prop *prop, Gfx *gdl, bool xlupass)
{
	...
	if (shademode != SHADEMODE_XLU && alpha > 0) {
		...
		if (g_Vars.normmplayerisrunning) {
			highlightcolour = scenarioHighlightProp(prop, colour);
		}
		...
		renderdata.envcolour = chrbloodstaincolour[0] << 24 | chrbloodstaincolour[1] << 16 | chrbloodstaincolour[2] << 8; //Set blood stain color
				...
				if (cheap || g_ExtraBoundsDist > 0.0f) {
					...
					while (hitpart > 0) {
						if (func0f084594(model, node, &shotdata->gunpos2d, &shotdata->gundir2d, &sp88, &sp84, &sp80)) {
							...
							mtx4RotateVecInPlace(camGetProjectionMtx(), &spd0);
							break;
						}
					...
					if (hitpart > 0) {
						if (func0f06bea0(model, model->definition->rootnode, model->definition->rootnode, &shotdata->gunpos2d,
							...
							mtx4TransformVec(camGetProjectionMtx(), &sp88.pos, &spdc);
							mtx4RotateVec(camGetProjectionMtx(), &sp88.unk0c, &spd0);
						} else {
							hitpart = 0;
						}
					}
...
void chrHit(struct shotdata *shotdata, struct hit *hit)
{
	...
	if ((chr->chrflags & CHRCFLAG_HIDDEN) == 0) {
		...
		mtx4TransformVec(camGetProjectionMtx(), &sp98, &hitpos);
		...
		if (shield <= 0) {
			if (hit->prop->type == PROPTYPE_WEAPON) {
				...
				// Create blood
				mtxInvertRigidBodyMatrix((Mtx*)sp58, (Mtx*)&spb0);
		...
		struct modelnode *node, int side, int arg6, int arg7, int alpha)
{
	...
	if (side == -7) {
		...
		chrCalcShieldColor(shield, &red1, &green1, &blue1);
		...
		st1 = (sinf((g_Vars.thisframestart240 % TICKS(1200)) * 0.005235154f) + 1.0f) * 0.5f * 32.0f * 32.0f;
		st2 = (cosf((g_Vars.thisframestart240 % TICKS(1200)) * 0.005235154f) + 1.0f) * 0.5f * 32.0f * 32.0f;
		st3 = st1 + 512;
		st4 = st2 + 512;
