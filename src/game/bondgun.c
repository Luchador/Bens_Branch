...
void bgunUpdateHandState2(struct hand *hand, int handnum, struct coord *viewmodelpos, struct weaponfunc *funcdef, Mtxf *arg4, Mtxf *arg5)
{
	...
	mtxApplyAffineInPlace(arg4, arg5);
...
void bgunCreateXBowBolt(struct defaultobj *obj, struct coord *coord, RoomNum *rooms, Mtxf *matrix1, struct coord *velocity, Mtxf *matrix2, struct prop *prop, struct coord *pos)
{
	...
	if (objprop) {
		...
		propEnable(objprop);
		mtxScaleRotationAndTranslation(obj->model->scale, matrix1);
				...
				if (weapon->base.model && weapon->base.model->definition) {
					...
					if (weapon->base.hidden & OBJHFLAG_PROJECTILE) {
						...
						if (funcdef->scale != 1.0f) {
							...
							mtxScaleRotationAndTranslation(funcdef->scale, &sp78);
	...
	if (hand->matmot1 > 0.0f) {
		...
		*arg2 = true;
		...
		if (node) {
			...
			mtxScaleRowZ((1.0f - hand->matmot1) * 8.0f + 0.5f, (Mtxf *)((uintptr_t)allocation + sp34 * sizeof(Mtxf)));
			mtxScaleRowX((1.0f - hand->matmot1) * 3.0f + 1.0f, (Mtxf *)((uintptr_t)allocation + sp34 * sizeof(Mtxf)));
			mtxScaleRowY((1.0f - hand->matmot1) * 3.0f + 1.0f, (Mtxf *)((uintptr_t)allocation + sp34 * sizeof(Mtxf)));
		}
	}
}
	...
	if (modeldef != NULL) {
		for (i = 0; i < hand->unk0cc8_04; i++) {
			...
			if (node) {
				...
				mtx4Copy(tmp, &sp4c);
				mtxScaleRotationAndTranslation(9.999999f, &sp4c);
		...
		bool **arg7, int mtxindex, Mtxf *arg9, Mtxf *arg10)
{
	...
	mtxScaleRotationAndTranslation(spb4, &spd8);
	mtxScaleRowZ(muzzlez, &spd8);
	mtx4Copy(&spd8, mtx);
	...
	for (partnum = 0x50; partnum <= 0x52; partnum++) {
		...
		if (node && weaponnum != WEAPON_REAPER && weaponnum != WEAPON_SHOTGUN) {
			...
			mtxScaleRotationAndTranslation(0.10000001f * spb4, &sp70);
	...
	if (funcdef) {
		...
		if (modeldef && weaponnum != WEAPON_DY357MAGNUM && weaponnum != WEAPON_DY357LX) {
			...
			if (node) {
				...
				mtx4Copy(mtx, &sp24);
				mtxScaleRotationAndTranslation(9.999999f, &sp24);
...
void bgun0f0a5550(int handnum)
{
	...
	if (hand->useposrot) {
		...
		mtxApplyAffineInPlace(&hand->posrotmtx, &sp234);
	...
	mtxApplyAffineInPlace(&sp284, &sp234);
	...
	if (hand->visible) {
		...
		if (weaponHasFlag(weaponnum, WEAPONFLAG_DUALFLIP) && handnum == HAND_LEFT) {
			mtxScaleXAxis(-1, &sp2c4);
		}

		mtxScaleRotationAndTranslation(0.10000001f, &sp2c4);
