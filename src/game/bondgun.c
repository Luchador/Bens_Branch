...
void bgunSetArmPitch(struct hand *hand, float angle)
{
	...
	hand->posrotmtx[3][0] = 0;
	hand->posrotmtx[3][1] = (1.0f - cosf(angle)) * -80.0f;
	hand->posrotmtx[3][2] = sinf(angle) * 15.0f;
}
...
void bgunUpdateHandState2(struct hand *hand, int handnum, struct coord *viewmodelpos, struct weaponfunc *funcdef, Mtx *rotMtx, Mtx *baseMtx)
{
	float tmp;
	...
	mtx4LoadRotation(&sp38, rotMtx);
	mtxApplyAffineTransformInPlace(rotMtx, baseMtx);
...
void bgunUpdateBlend(struct hand *hand, int handnum)
{
	...
	utilsCatmullRomSplineInterp(&hand->blendpos[(pos + 3) % 4], &hand->blendpos[pos], &hand->blendpos[(pos + 1) % 4], &hand->blendpos[(pos + 2) % 4], hand->dampt, &sp5c);
	utilsCatmullRomSplineInterp(&hand->blendlook[(pos + 3) % 4], &hand->blendlook[pos], &hand->blendlook[(pos + 1) % 4], &hand->blendlook[(pos + 2) % 4], hand->dampt, &sp50);
	utilsCatmullRomSplineInterp(&hand->blendup[(pos + 3) % 4], &hand->blendup[pos], &hand->blendup[(pos + 1) % 4], &hand->blendup[(pos + 2) % 4], hand->dampt, &sp44);
	...
	for (i = 0; i < g_Vars.lvupdate240; i++) {
		hand->damppossum.x = 0.9872f * hand->damppossum.x + sp5c.f[0];
		hand->damppossum.y = 0.9872f * hand->damppossum.y + sp5c.f[1];
		hand->damppossum.z = 0.9872f * hand->damppossum.z + sp5c.f[2];

		hand->damplooksum.x = 0.9872f * hand->damplooksum.x + sp50.f[0];
		hand->damplooksum.y = 0.9872f * hand->damplooksum.y + sp50.f[1];
		hand->damplooksum.z = 0.9872f * hand->damplooksum.z + sp50.f[2];

		hand->dampupsum.x = 0.9872f * hand->dampupsum.x + sp44.f[0];
		hand->dampupsum.y = 0.9872f * hand->dampupsum.y + sp44.f[1];
		hand->dampupsum.z = 0.9872f * hand->dampupsum.z + sp44.f[2];
	}

	hand->damppos.x = hand->damppossum.x * 0.012799978f * 2;
	hand->damppos.y = hand->damppossum.y * 0.012799978f * 2;
	hand->damppos.z = hand->damppossum.z * 0.012799978f * 2;

	hand->damplook.x = hand->damplooksum.x * 0.012799978f;
	hand->damplook.y = hand->damplooksum.y * 0.012799978f;
	hand->damplook.z = hand->damplooksum.z * 0.012799978f;

	hand->dampup.x = hand->dampupsum.x * 0.012799978f;
	hand->dampup.y = hand->dampupsum.y * 0.012799978f;
	hand->dampup.z = hand->dampupsum.z * 0.012799978f;
}
			...
			if (player->gunctrl.masterloadstate != MASTERLOADSTATE_LOADED || newweaponnum != player->gunctrl.gunmemtype) {
				if (filenum) {
					...
					if (player->gunctrl.masterloadstate == MASTERLOADSTATE_FLUX) {
						...
						if (player->gunctrl.memloadremaining > 50 * sizeof(Mtx)) {
							hand->unk0dd8 = (Mtx *) player->gunctrl.memloadptr;
							player->gunctrl.memloadptr += 50 * sizeof(Mtx);
							player->gunctrl.memloadremaining -= 50 * sizeof(Mtx);
						} else {
							hand->unk0dd8 = NULL;
						}
...
{
	for (int i = 0; i < g_Vars.lvupdate240; i += 8) {
		bgunTickMasterLoad();
	}
}
...
void bgunCreateXBowBolt(struct defaultobj *obj, struct coord *coord, RoomNum *rooms, Mtx *matrix1, struct coord *velocity, Mtx *matrix2, struct prop *prop, struct coord *pos)
{
	...
	if (objprop) {
		...
		propEnable(objprop);
		mtxScaleRotationPart(obj->model->scale, matrix1);
		...
		if (obj->hidden & OBJHFLAG_PROJECTILE) {
			...
			memcpy(&obj->projectile->mtx, matrix2, sizeof(Mtx));
...
void bgun0f09ed2c(struct defaultobj *obj, struct coord *newpos, Mtx *arg2, struct coord *velocity, Mtx *arg4)
{
	struct prop *objprop = obj->prop;
	struct coord pos;
	RoomNum rooms[8];
...
struct defaultobj *bgunCreateThrownProjectile2(struct chrdata *chr, struct gset *gset, struct coord *pos, RoomNum *rooms, Mtx *handMtx, struct coord *velocity)
{
	...
	if (gset->weaponnum == WEAPON_COMBATKNIFE) {
		mtxRotate(&mtx, 90.0f / (RANDOMFRAC() + 12.1f),
				(*handMtx)[1][0], (*handMtx)[1][1], (*handMtx)[1][2]); // Spin knife around local Y-axis
	} else {
	...
	if (obj != NULL) {
		bgunCreateXBowBolt(obj, pos, rooms, handMtx, velocity, &mtx, chr->prop, pos);
...
void bgunCreateThrownProjectile(int handnum, struct gset *gset)
{
	...
	mtxIdent(&transformMtx);

	if (gset->weaponnum == WEAPON_COMBATKNIFE) {
		mtx4LoadZRotation(4.711639f, &transformMtx);
		mtx4LoadXRotation(3.1410925f, &muzzleMtx);
		mtx4MultMtx4InPlace(&muzzleMtx, &transformMtx);
	}

	memcpy(&muzzleMtx, &g_Vars.currentplayer->hands[handnum].muzzlemat, sizeof(Mtx));

	utilsNormalizeF(&muzzleMtx[0][0], &muzzleMtx[0][1], &muzzleMtx[0][2]);
	utilsNormalizeF(&muzzleMtx[1][0], &muzzleMtx[1][1], &muzzleMtx[1][2]);
	utilsNormalizeF(&muzzleMtx[2][0], &muzzleMtx[2][1], &muzzleMtx[2][2]);
	...
	mtx4RotateVecInPlace(camGetProjectionMtx(), &gundir);
	...
	if (droppinggrenade) {
		...
		if (hand->hasdotinfo) {
			...
			chrCalculateTrajectory(&spawnpos, 21.666666f, &aimpos, &targetDir);

			radians = acosf(gundir.f[0] * targetDir.f[0] + gundir.f[1] * targetDir.f[1] + gundir.f[2] * targetDir.f[2]);

			Mtx targetMtxTemp;

			// Check within 20 degrees
			if (radians > 0.34901026f || radians < -0.34901026f) {
				mtxBuildLookAtMatrix2F(&aimMtx1, 0, 0, 0, gundir.x, gundir.y, gundir.z, 0, 1, 0);
				mtxBuildLookAtMatrix2F(&targetMtxTemp, 0, 0, 0, targetDir.x, targetDir.y, targetDir.z, 0, 1, 0);

				quaternion3x3MtxToQuat(&aimMtx1, startQuat);
				quaternion3x3MtxToQuat(&targetMtxTemp, targetQuat);
				quaternionAvoidFlips(startQuat, targetQuat);
				...
				quaternionSlerp(startQuat, targetQuat, frac, interpQuat);
				quaternionToMtx(interpQuat, &aimMtx2);

				gundir.x = -aimMtx2[2][0];
				gundir.y = -aimMtx2[2][1];
				gundir.z = -aimMtx2[2][2];
			} else {
				gundir.x = targetDir.x;
				gundir.y = targetDir.y;
				gundir.z = targetDir.z;
			}
		}
	...
	obj = bgunCreateThrownProjectile2(g_Vars.currentplayer->prop->chr, gset, &spawnpos, spawnrooms, &transformMtx, &velocity);
...
void bgunUpdateHeldRocket(int handnum)
{
	...
	if (obj) {
		...
		if (objprop) {
			...
			if (!hand->firedrocket) {
				memcpy(&mtx, &hand->posmtx, sizeof(Mtx));
...
void bgunCreateFiredProjectile(int handnum)
{
	...
	if (weapondef) {
		...
		if (tmp && tmp->type == INVENTORYFUNCTYPE_SHOOT_PROJECTILE) {
			...
			if (gsetHasFunctionFlags(&hand->gset, FUNCFLAG_CALCULATETRAJECTORY)) {
				...
				if (hand->hasdotinfo) {
					...
					if (radians > 0.17450513f || radians < -0.17450513f) {
						...
						quaternion3x3MtxToQuat(&sp174, spe4);
						quaternion3x3MtxToQuat(&sp134, spd4);
			...
			memcpy(&sp210, &g_Vars.currentplayer->hands[handnum].posmtx, sizeof(Mtx));
				...
				if (weapon->base.model && weapon->base.model->definition) {
					...
					if (weapon->base.hidden & OBJHFLAG_PROJECTILE) {
						...
						if (funcdef->scale != 1.0f) {
							...
							mtxScaleRotationPart(funcdef->scale, &sp78);
...
void bgunSwivel(float screenx, float screeny, float crossdamp, float aimdamp)
{
	...
	for (h = 0; h < 1; h++) {
		if (!ignore[h]) {
			...
			if (hand->hasdotinfo && !g_Vars.mplayerisrunning) {
				...
				mtx4TransformVecInPlace(camGetPlayerWorldToScreenMtx(), &sp94);
	...
	for (l = 0; l < g_Vars.lvupdate240; l++) {
		...
		for (h = 0; h < 2; h++) {
			hand = &player->hands[h];
			hand->guncrosspossum[0] = 0.9269697f * hand->guncrosspossum[0] + x[h];
			hand->guncrosspossum[1] = 0.9269697f * hand->guncrosspossum[1] + y[h];
		}
	}
	...
	for (h = 0; h < 2; h++) {
		player->hands[h].crosspos[0] = player->hands[h].guncrosspossum[0] * 0.07303029f * screenwidth * 0.5f + screenwidth * 0.5f;
		player->hands[h].crosspos[1] = player->hands[h].guncrosspossum[1] * 0.07303029f * screenheight * 0.5f + screenheight * 0.5f;
	...
	camProjectScreenToWorldDir(player->crosspos2, &aimpos, 1000);
...
void bgunSwivelWithDamp(float screenx, float screeny, float crossdamp)
{
	...
	float aimdamp = weapon->aimsettings->aimdamp;
...
void bgunSwivelWithoutDamp(float screenx, float screeny)
{
	...
	bgunSwivel(screenx, screeny, 0.945f, aimdamp);
}
...
void bgun0f0a0c08(struct coord *arg0, struct coord *arg1)
{
	...
	camProjectScreenToWorldDir(g_Vars.currentplayer->crosspos, arg1, 1);
}
...
void bgun0f0a0c44(int handnum, struct coord *arg1, struct coord *arg2)
{
	...
	camProjectScreenToWorldDir(g_Vars.currentplayer->hands[handnum].crosspos, arg2, 1);
}
...
void bgunCalculatePlayerShotSpread(struct coord *gunpos2d, struct coord *gundir2d, int handnum, bool dorandom)
{
	...
	camProjectScreenToWorldDir(crosspos, gundir2d, 1);
}
...
void bgunUpdateGangsta(struct hand *hand, int handnum, struct coord *viewmodelpos, struct weaponfunc *funcdef, Mtx *rotMtx, Mtx *baseMtx)
{
	float tmp;
	struct coord sp38 = {0, 0, 0};
...
void bgunUpdateLasersight(struct hand *hand, struct modeldef *modeldef, int handnum, uint8_t *allocation)
{
    ...
    if (node) {
		...
		Mtx *matrices = (Mtx *)allocation;

		memcpy(&beamnear, matrices[mtxindex][3], sizeof(struct coord));

		mtx4TransformVecInPlace(camGetProjectionMtx(), &beamnear);
		...
		if (hand->useposrot
			...
			mtx4TransformVec(camGetPlayerWorldToScreenMtx(), &sp3c, &sp54);
			mtx4RotateVec(camGetProjectionMtx(), &sp48, &sp30);

			beamfar.x *= 500.0f;
			beamfar.y *= 500.0f;
			beamfar.z *= 500.0f;

			mtx4RotateVecInPlace(camGetProjectionMtx(), &beamfar);
		...
		if (busy) {
			...
			mtx4TransformVecInPlace((Mtx *)((uintptr_t)allocation + mtxindex * sizeof(Mtx)), &beamfar);
		} else {
			camProjectScreenToWorldDir(g_Vars.currentplayer->crosspos, &beamfar, 1);
		...
		mtx4TransformVecInPlace(camGetProjectionMtx(), &beamfar);
		lasersightSetBeam(handnum, 1, &beamnear, &beamfar);
...
void bgunUpdateSniperRifle(struct modeldef *modeldef, uint8_t *allocation)
{
	...
	float scopeOffsets[4] = {0, 0, 0, 0};
	float zoomFactor;
	int mtxindex;
	struct coord offset;

	// Zoom factor ranges from 0 (max zoom) to 1 (no zoom)
	zoomFactor = 1.0f - (currentPlayerGetGunZoomFov() - 2.0f) / 58.0f;
	...
	for (int i = 0; i < ARRAYCOUNT(nodes); i++) {
		if (nodes[i]) {
			float partialZoom = zoomFactor * 4.0f;
			scopeOffsets[i] = partialZoom - i;

			if (partialZoom < i) {
				scopeOffsets[i] = 0.0f;
			}

			scopeOffsets[i] *= 100.0f;

			offset.x = 0.0f;
			offset.y = 0.0f;
			offset.z = scopeOffsets[i];

			mtxindex = modelFindNodeMtxIndex(nodes[i], 0);
			Mtx *matrix = (Mtx *)((uintptr_t)allocation + mtxindex * sizeof(Mtx));

			// Transform the offset by the rotation part of the matrix
			mtx4RotateVecInPlace(matrix, &offset);

			// Translate the matrix by the rotated offset
			(*matrix)[3][0] += offset.x;
			(*matrix)[3][1] += offset.y;
			(*matrix)[3][2] += offset.z;
		}
...
void bgunUpdateDevastator(struct hand *hand, uint8_t *allocation, struct modeldef *modeldef)
{
	...
	if (node) {
		...
		mtx4RotateVecInPlace(matrix, &sp24);
	...
	if (hand->matmot1 > 0.0f) {
		...
		*arg2 = true;
		...
		if (node) {
			...
			mtxScaleRow2Full((1.0f - hand->matmot1) * 8.0f + 0.5f, (Mtx *)((uintptr_t)allocation + sp34 * sizeof(Mtx)));
			mtxScaleRow0Full((1.0f - hand->matmot1) * 3.0f + 1.0f, (Mtx *)((uintptr_t)allocation + sp34 * sizeof(Mtx)));
			mtxScaleRow1Full((1.0f - hand->matmot1) * 3.0f + 1.0f, (Mtx *)((uintptr_t)allocation + sp34 * sizeof(Mtx)));
		}
	}
}
 ...
 */
void bgunUpdateMagnum(struct hand *hand, int handnum, struct modeldef *modeldef, Mtx *mtx)
{
	...
	if (modeldef != NULL) {
		for (i = 0; i < hand->unk0cc8_04; i++) {
			...
			if (node) {
				...
				memcpy(sp4c, tmp, sizeof(Mtx));
				mtxScaleRotationPart(10.0f, &sp4c);
				mtx4MultMtx4InPlace(camGetProjectionMtx(), &sp4c);
...
/**
 * Handle ejected parts like grenade pins and tranquilizer packs
 */
void bgunTickEject(struct hand *hand, struct modeldef *modeldef, bool isdetonator)
{
	float lvupdate;
	struct coord rotationAngles;
	struct coord movementVelocity;
	float nextVerticalVelocity;
	float grenadeForceScale = 3;

	switch (hand->ejectstate) {
	case EJECTSTATE_INIT:
		switch (hand->ejecttype) {
		case EJECTTYPE_GUN:
			hand->unk0d20.x = (RANDOMFRAC() - 0.5f) * 0.5333333f * 0.0625f + 0.5333333f;
			hand->unk0d20.y = RANDOMFRAC() * 2.5f * 0.0625f + 2.5f;
			hand->unk0d20.z = 0.0f;
			rotationAngles.x = RANDOMFRAC() * 2.0f * M_TAU / 184.0f - 0.03414231f;
			rotationAngles.y = RANDOMFRAC() * 2.0f * M_TAU / 184.0f - 0.03414231f;
			rotationAngles.z = RANDOMFRAC() * 2.0f * M_TAU / 184.0f - 0.03414231f;
			break;
		case EJECTTYPE_GRENADEPIN:
			hand->unk0d20.x = -((RANDOMFRAC() - 0.5f) * 0.5333333f * 0.0625f + grenadeForceScale * 0.5333333f);
			hand->unk0d20.y = RANDOMFRAC() * 2.5f * 0.125f + 2.5f;
			hand->unk0d20.z = -(RANDOMFRAC() + 1.0f);
			rotationAngles.x = (RANDOMFRAC() + 3.0f) * M_TAU / 208.0f;
			rotationAngles.y = RANDOMFRAC() * 2.0f * M_TAU / 544.0f - 0.0115481345f;
			rotationAngles.z = RANDOMFRAC() * 2.0f * M_TAU / 544.0f - 0.0115481345f;
			break;
		case EJECTTYPE_TRANQCASE:
			hand->unk0d20.x = 0.0f;
			hand->unk0d20.y = RANDOMFRAC() * 2.5f * 0.125f + 2.5f;
			hand->unk0d20.z = (RANDOMFRAC() + 1.0f) * 0.25f;
			rotationAngles.x = (RANDOMFRAC() + 3.0f) * M_TAU / 368.0f;
			rotationAngles.y = RANDOMFRAC() * 2.0f * M_TAU / 944.0f - 0.006654857f;
			rotationAngles.z = RANDOMFRAC() * 2.0f * M_TAU / 944.0f - 0.006654857f;
			break;
		}
		...
		Mtx rotationMtx;
		mtx4LoadRotation(&rotationAngles, &rotationMtx);
		mtx4ToMtx3(&rotationMtx, hand->unk0d50);

		if (g_Vars.lvupdate240 > 0 && hand->ejecttype != EJECTTYPE_GUN) {
			movementVelocity.x = (hand->posmtx[3][0] - hand->prevmtx[3][0]) / g_Vars.lvupdate60freal;
			movementVelocity.y = (hand->posmtx[3][1] - hand->prevmtx[3][1]) / g_Vars.lvupdate60freal;
			movementVelocity.z = (hand->posmtx[3][2] - hand->prevmtx[3][2]) / g_Vars.lvupdate60freal;

			Mtx invMtx;
			mtxFullInverse4x4(&hand->posmtx, &invMtx);
			mtx4RotateVecInPlace(&invMtx, &movementVelocity);

			hand->unk0d20.x += movementVelocity.x * 0.3f;
			hand->unk0d20.y += movementVelocity.y * 0.3f;
			hand->unk0d20.z += movementVelocity.z * 0.3f;
		}
	...
	case EJECTSTATE_AIRBORNE:
		...
		nextVerticalVelocity = hand->unk0d20.y - lvupdate * 0.2777778f;
		...
		hand->unk0d18 += lvupdate * 0.5f * (hand->unk0d20.y + nextVerticalVelocity);
		hand->unk0d14 += lvupdate * hand->unk0d20.x;
		hand->unk0d1c += lvupdate * hand->unk0d20.z;

		hand->unk0d20.y = nextVerticalVelocity;

		for (int i = 0; i < g_Vars.lvupdate240; i++) {
...
void bgunCreateFx(struct hand *hand, int handnum, struct weaponfunc *funcdef, int weaponnum, struct modeldef *modeldef, uint8_t *allocation)
{
	...
	if (funcdef) {
		...
		if (modeldef && weaponnum != WEAPON_DY357MAGNUM && weaponnum != WEAPON_DY357LX) {
			...
			if (node) {
				Mtx *mtx = (Mtx*)allocation;
				Mtx sp24;

				mtx += modelFindNodeMtxIndex(node, 0);

				mtx4Copy(mtx, &sp24);
				mtxScaleRotationPart(10.0f, &sp24);
				mtx4MultMtx4InPlace(camGetProjectionMtx(), &sp24);
...
void bgunTickHandWeapModel(int handnum)
{
	...
	if (hand->visible) {
		...
		mtxallocation = gfxAllocate(modeldef->nummatrices * sizeof(Mtx));

		if (weaponHasFlag(weaponnum, WEAPONFLAG_02000000)) {
			for (int i = 0; i < modeldef->nummatrices; i++) {
				mtx = (Mtx *)(mtxallocation + i * sizeof(Mtx));
				mtxIdent(mtx);
			}
		}
	...
	// Ben's comment: weapons with the turn up flag check for a nearby wall to see if they can be turned up
	...
	if (hand->useposrot) {
		viewmodelpos.f[0] += hand->posrotmtx[3][0];
		viewmodelpos.f[1] += hand->posrotmtx[3][1];
		viewmodelpos.f[2] += hand->posrotmtx[3][2];

		mtxApplyAffineTransformInPlace(&hand->posrotmtx, &sp234);
	...
	mtxApplyAffineTransform(camGetProjectionMtx(), &hand->cammtx, &hand->posmtx);
	...
	if (hand->visible) {
		...
		mtxScaleRotationPart(0.10f, &sp2c4);

		mtx4Copy(&sp2c4, (Mtx*)mtxallocation);
		...
		if (node) {
			...
			hand->muzzlepos.f[0] = (*mtx)[3][0];
			hand->muzzlepos.f[1] = (*mtx)[3][1];
			hand->muzzlepos.f[2] = (*mtx)[3][2];
			...
			hand->muzzlepos.x = (*mtx)[3][0];
			hand->muzzlepos.y = (*mtx)[3][1];
			hand->muzzlepos.z = (*mtx)[3][2];
			...
			hand->muzzlepos.z = hand->posmtx[3][2];

			mtx4Copy(&hand->posmtx, &hand->muzzlemat);

			hand->muzzlez = -hand->cammtx[3][2];
		}
	...
	case WEAPON_DY357LX:
		if (hand->unk0cc8_04 > 0) {
			bgunUpdateMagnum(hand, handnum, modeldef, (Mtx *)mtxallocation);
		}
		break;
	}
	...
	for (i = 0; i < 2; i++) {
		...
		if (hand->visible) {
			...
			if (weaponHasFlag(hand->gset.weaponnum, WEAPONFLAG_00008000)) {
				gfx_Set_Lights1(gdl++, &g_GunLight);
				gfx_LookAt(gdl++, camGetLookAt());
			}
			...
			if (weaponHasFlag(weaponnum, WEAPONFLAG_DUALFLIP)) {
				gfx_Clear_Geometry_Mode(renderdata.gdl++, G_CULL_BOTH);
