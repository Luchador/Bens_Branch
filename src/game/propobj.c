...
float g_GasReleaseTimerMax240;
bool g_GasEnableDamage;
int g_MaxWeaponSlots;
int g_MaxHatSlots;
int g_MaxAmmoCrates;
int g_MaxDebrisSlots;
int g_MaxProjectiles;
int g_MaxEmbedments;
...
uint32_t g_TintedGlassEnabled = 0;
int g_AlarmTimer = 0;
struct sndstate *g_AlarmAudioHandle = NULL;
float g_AlarmSpeakerWeight = 64;
float g_AlarmSpeakerDirection = 1;
float g_GasReleaseTimer240 = 0;
bool g_GasReleasing = false;
struct coord g_GasPos = {0};
int g_GasLastCough60 = 0;
float g_GasSoundTimer240 = 0;
struct sndstate *g_GasAudioHandle = NULL;
uint32_t g_CountdownTimerOff = COUNTDOWNTIMERREASON_AI;
bool g_CountdownTimerRunning = false;
float g_CountdownTimerValue60 = 0;
uint32_t g_PlayersDetonatingMines = 0;
int g_NextWeaponSlot = 0;
int g_NextHatSlot = 0;
...
int g_EmbedHitPart = 0;
uint32_t g_EmbedSide = 0;
int16_t var8006993c[3] = {0};
uint32_t g_HitTextureNum = 0;
float g_CctvWaitScale = 1;
float g_CctvDamageRxScale = 1;
float g_AutogunAccuracyScale = 1;
float g_AutogunDamageTxScale = 1;
float g_AutogunDamageRxScale = 1;
float g_AmmoQuantityScale = 1;
struct padeffectobj *g_PadEffects = NULL;
int g_LastPadEffectIndex = -1;
struct autogunobj *g_ThrownLaptops = NULL;
struct beam *g_ThrownLaptopBeams = NULL;
int g_MaxThrownLaptops = 0;
	...
	if (door->base.hidden & OBJHFLAG_LIFTDOOR) {
		...
		while (link) {
			if (doorprop == link->door && link->lift
				...
				if (link->lift->obj->type == OBJTYPE_DOOR) {
				...
				} else if (link->lift->obj->type == OBJTYPE_LIFT) {
					if (allowclose
						...
						bool vacant = true;
						int numchrslots = chrsGetNumSlots();
						int i;
...
int objCalculateGeoBlockVertices(float xmin, float xmax, float ymin, float ymax, float zmin, float zmax, Mtx *mtx, struct geoblock *block)
{
	...
	double mtx00min = mtx00 * xmin64;
	double mtx02min = mtx02 * xmin64;
	double mtx10min = mtx10 * ymin64;
	double mtx12min = mtx12 * ymin64;
	double mtx20min = mtx20 * zmin64;
	double mtx22min = mtx22 * zmin64;
	double mtx00max = mtx00 * xmax64;
	double mtx02max = mtx02 * xmax64;
	double mtx10max = mtx10 * ymax64;
	double mtx12max = mtx12 * ymax64;
	double mtx20max = mtx20 * zmax64;
	double mtx22max = mtx22 * zmax64;
	...
	for (i = 0; i < numverts; i++) {
		block->vertices[i][0] += (*mtx)[3][0];
		block->vertices[i][1] += (*mtx)[3][2];
	}
...
void objCalculateGeoBlockFromBboxAndMtx(struct modelrodata_bbox *bbox, Mtx *mtx, struct geoblock *block)
{
	...
	block->ymin = (*mtx)[3][1] + objGetRotatedLocalYMinByMtx4(bbox, mtx);
	block->ymax = (*mtx)[3][1] + objGetRotatedLocalYMaxByMtx4(bbox, mtx);
}
...
{
	for (int i = 0; i < rodata19->numvertices; i++) {
		block->vertices[i][0] = (*mtx)[3][0] + (*mtx)[0][0] * rodata19->vertices[i].x + (*mtx)[1][0] * rodata19->vertices[i].y + (*mtx)[2][0] * rodata19->vertices[i].z;
		block->vertices[i][1] = (*mtx)[3][2] + (*mtx)[0][2] * rodata19->vertices[i].x + (*mtx)[1][2] * rodata19->vertices[i].y + (*mtx)[2][2] * rodata19->vertices[i].z;
	}
...
{
	float xdiff = coord->x - pos->x;
	float ydiff = coord->y - pos->y;
	float zdiff = coord->z - pos->z;
	float f0;
...
bool objTestModelHit(struct model *model, float *max, float *min, float arg3[2], float arg4[2])
{
	...
	while (node) {
		...
		if (type == MODELNODETYPE_BBOX) {
			...
			dist1 = objGetRotatedLocalXMaxByMtx4(bbox, mtx) + (*mtx)[3][0];
			dist2 = objGetRotatedLocalXMinByMtx4(bbox, mtx) + (*mtx)[3][0];

			if (arg3[0] - arg4[0] <= dist1 && arg3[0] + arg4[0] >= dist2) {
				dist1 = objGetRotatedLocalYMaxByMtx4(bbox, mtx) + (*mtx)[3][1];
				dist2 = objGetRotatedLocalYMinByMtx4(bbox, mtx) + (*mtx)[3][1];

				if (arg3[1] - arg4[1] <= dist1 && arg3[1] + arg4[1] >= dist2) {
					dist1 = objGetRotatedLocalZMaxByMtx4(bbox, mtx) + (*mtx)[3][2];
					dist2 = objGetRotatedLocalZMinByMtx4(bbox, mtx) + (*mtx)[3][2];
...
void func0f06803c(struct coord *arg0, float *arg1, float *arg2, float *arg3, float *arg4)
{
	...
	float aspect = videoGetAspect();
	float fovy = viGetFovY();
	...
	camProjectWithZoomAndAspect(&sp4c, sp44, fovy, aspect);
...
int objGetAverageBrightnessInRooms(RoomNum *rooms, int brightnesstype)
{
	int brightness = 0;
	int i;
...
int door0f068c04(struct prop *prop, int *arg1, int *arg2)
{
	...
	int i;
	int br_settled_average;
	int total_br_settled = 0;
	int total_br_flash = 0;
	int s2 = 0;
	int s5 = 0;
	int br_flash_average;
	int result;
	struct prop *loopprop;
	struct pad pad;

	static uint32_t debugdoors = 0;
			...
			if (g_Vars.currentplayer->prop) {
				...
				if (door->base.flags3 & OBJFLAG3_04000000) {
					...
					for (i = 0; loopprop->rooms[i] != -1; i++) {
						float roomx = g_Rooms[loopprop->rooms[i]].centre.x;
						float roomy = g_Rooms[loopprop->rooms[i]].centre.y;
						float roomz = g_Rooms[loopprop->rooms[i]].centre.z;
						float xdist = roomx - pos->x;
						float ydist = roomy - pos->y;
						float zdist = roomz - pos->z;
						...
						if ((sum1 < 0.0f && sum1 < 0.0f) || (sum1 > 0.0f && sum2 > 0.0f)) {
							int value1 = roomGetFlashBrightness(loopprop->rooms[i]);
							int value2 = roomGetSettledRegionalBrightnessForPlayer(loopprop->rooms[i]);
...
void propCalculateShadeColour(struct prop *prop, uint8_t *nextcol, uint16_t floorcol)
{
	...
	static uint32_t scol = 0x00;
	static uint32_t salp = 0x00;
	...
	if (obj && (obj->flags & OBJFLAG_IGNOREFLOORCOLOUR)) {
		...
		uint8_t r = ((floorcol >> 8) & 0xf);
		uint8_t g = ((floorcol >> 4) & 0xf);
		uint8_t b = ((floorcol >> 0) & 0xf);
	...
	if (nextcol[max] > 0) {
		int tmp = nextcol[med] * (nextcol[max] - nextcol[min]) / nextcol[max];
		int range = nextcol[max] - nextcol[min];
		nextcol[min] = 0;
		nextcol[med] = tmp;
		nextcol[max] = range;
	}
...
void colourTween(uint8_t *col, uint8_t *nextcol)
{
	...
	for (i = 0; i < 4; i++) {
		int remaining = nextcol[i] - col[i];
		int newcol = col[i] + (remaining >> 1);
		col[i] = newcol;
	}
}
...
void func0f069b4c(struct defaultobj *obj)
{
	...
	uint8_t *ptr = (uint8_t *) obj->unkgeo;

	if (ptr != NULL) {
		if ((obj->hidden2 & OBJH2FLAG_DYNAMIC_COLLISION)) {
			if (obj->flags3 & OBJFLAG3_GEOCYL) {
		...
		if (rodata != NULL) {
			uint32_t flags = GEOFLAG_FLOOR1 | GEOFLAG_FLOOR2;
		...
		*debris = tmp;
		...
		if (objInitWithModelDef(debris, g_ModelStates[debris->modelnum].modeldef)) {
			...
			if (debris->hidden & OBJHFLAG_PROJECTILE) {
				...
				mtx4LoadRotation(&rot, (Mtx*)&projectile->mtx);
			}
...
struct prop *objInit(struct defaultobj *obj, struct modeldef *modeldef, struct prop *prop, struct model *model)
{
	...
	if (prop && model) {
		...
		if (obj->flags & OBJFLAG_GEOMETRY_TEMP_ENABLED) {
			if (obj->flags3 & OBJFLAG3_GEOCYL) {
			...
			obj->geocount++;
			obj->hidden2 |= OBJH2FLAG_DYNAMIC_COLLISION;
		} else {
			obj->hidden2 &= ~OBJH2FLAG_DYNAMIC_COLLISION;
		}
...
// Add a small vertical offset for non-weapons
...
void func0f06a730(struct defaultobj *obj, struct coord *arg1, Mtx *mtx, RoomNum *rooms, struct coord *centre)
{
	struct modelrodata_bbox *bbox = modelFindBboxRodata(obj->model);
	float min = objGetLocalYMin(bbox);
	float max = objGetLocalYMax(bbox);
	struct coord pos2;
	Mtx sp70;
	RoomNum rooms2[8];
	float curval;
	...
	if (obj->flags & OBJFLAG_UPSIDEDOWN) {
		...
		pos2.x = centre->x - sp70[1][0] * max;
		pos2.y = centre->y - sp70[1][1] * max;
		pos2.z = centre->z - sp70[1][2] * max;
	} else if (obj->flags & OBJFLAG_00000008) {
		mtx4Copy(mtx, &sp70);

		pos2.x = centre->x - sp70[1][0] * min;
		pos2.y = centre->y - sp70[1][1] * min;
		pos2.z = centre->z - sp70[1][2] * min;
	} else {
		mtx4Copy(mtx, &sp70);
		...
		pos2.x = centre->x - sp70[row][0] * min;
		pos2.y = centre->y - sp70[row][1] * min;
		pos2.z = centre->z - sp70[row][2] * min;

		propUpdatePositionRoomsSimple(arg1, rooms, &pos2, rooms2);
		...
		if (cdFindFloorRoomYColourFlagsAtPos(&pos2, rooms2, &y, &obj->floorcol, NULL) > 0)
		{
			...
			if (obj2) {
				...
				if (updated
						...
						&& block->ymax > y
						&& block->ymin < y + (max - min) * sp70[row][1] + objGetPlacementYOffset(obj)) {
					pos2.y = block->ymax - sp70[row][1] * min;
					obj->hidden |= OBJHFLAG_00008000;
				} else {
					pos2.y = y - min * sp70[row][1] + objGetPlacementYOffset(obj);
				}
			} else {
				pos2.y = y - min * sp70[row][1] + objGetPlacementYOffset(obj);
			}
...
void objPlaceObjectAligned(struct defaultobj *obj, struct coord *arg1, Mtx *arg2, RoomNum *rooms, struct coord *point)
{
	...
	newpos.x = point->x - sp5c[2][0] * mult;
	newpos.y = point->y - sp5c[2][1] * mult;
	newpos.z = point->z - sp5c[2][2] * mult;

	propUpdatePositionRoomsSimple(arg1, rooms, &newpos, newrooms);
...
void objFree(struct defaultobj *obj, bool freeprop, bool canregen)
{
	...
	if (obj->type == OBJTYPE_WEAPON) {
		...
		if (weapon->weaponnum == WEAPON_BOLT) {
			int beammnum = boltbeamFindByProp(obj->prop);
...
{
	struct coord edgeStart, edgeEnd;
	struct coord collisionPoint;
	float distanceAlongRay;

	// Check if the ray intersects the bounding volume of the prop
	if (!cdIsLineBlockedByProp(rayStart, rayEnd, targetProp)) {
		// If it intersects, retrieve collision edge and intersection point
		cdGetEdge(&edgeStart, &edgeEnd);
		cdGetPos(&collisionPoint);

		// Compute distance along ray to the intersection point using a dot product
		distanceAlongRay =
			(collisionPoint.f[0] - rayStart->f[0]) * rayDir->f[0] +
			(collisionPoint.f[1] - rayStart->f[1]) * rayDir->f[1] +
			(collisionPoint.f[2] - rayStart->f[2]) * rayDir->f[2];

		// Check if this hit is closer than any previously found hit
		if (distanceAlongRay < *closestHitDist) {
			*closestHitDist = distanceAlongRay;

			// Output hit position
			outHitPos->x = collisionPoint.x;
			outHitPos->y = collisionPoint.y;
			outHitPos->z = collisionPoint.z;

			// Set hit normal as the inverse of the projectile direction (projectile sticks "backward")
			outHitNormal->x = -rayDir->x;
			outHitNormal->y = 0.0f;
			outHitNormal->z = -rayDir->z;

			// Normalize the normal if needed (avoid zero vector)
			if (outHitNormal->x != 0.0f || outHitNormal->z != 0.0f) {
				utilsNormalizeF(&outHitNormal->x, &outHitNormal->y, &outHitNormal->z);
			} else {
				outHitNormal->z = 1.0f; // Default forward-facing normal
			}

			// Update global embed tracking state
			g_EmbedProp = targetProp;
			g_EmbedHitPart = 0;
			g_EmbedModel = NULL;
			g_EmbedNode = NULL;
	...
	if (sum1 >= -f0 && sum1 <= arg4 + f0) {
		if (prop->flags & PROPFLAG_ONTHISSCREENTHISTICK) {
			if (g_ExtraBoundsDist > 0.0f) {
				...
				while (hitpart > 0) {
					if (objTestAABBIntersection(model, spe4, arg5, arg6, &thing1, &mtxindex1, &node1)) {
						mtx4TransformVec((Mtx*)&model->matrices[mtxindex1], &thing1.pos, &spfc);
						...
						if (sum2 < *arg9) {
							mtx4RotateVec((Mtx*)&model->matrices[mtxindex1], &thing1.unk0c, &spf0);

							*arg9 = sum2;

							mtx4TransformVec(camGetProjectionMtx(), &spfc, arg7);
							mtx4RotateVec(camGetProjectionMtx(), &spf0, arg8);
				...
				if (obj->flags3 & OBJFLAG3_HOVERBEDSHIELD) {
					...
					if (node && objTestAABBIntersection(model, node, arg5, arg6, &thing2, &mtxindex2, &node2)) {
						if (hitpart <= 0 ||
								+ model->matrices[mtxindex2][0][2] * thing2.pos.f[0]
								+ model->matrices[mtxindex2][1][2] * thing2.pos.f[1]
								+ model->matrices[mtxindex2][2][2] * thing2.pos.f[2]
								>
								+ model->matrices[mtxindex1][0][2] * thing1.pos.f[0]
								+ model->matrices[mtxindex1][1][2] * thing1.pos.f[1]
								+ model->matrices[mtxindex1][2][2] * thing1.pos.f[2]
								) {
							hitpart = 1;
...
bool func0f06bea0(struct model *model, struct modelnode *endnode, struct modelnode *node, struct coord *arg3, struct coord *arg4, void *arg5, float *arg6, struct modelnode **arg7, int *hitpart, int *arg9, struct modelnode **arg10)
{
	...
	g_ExtraBoundsDist = 2.5f / model->scale;
		...
		case MODELNODETYPE_GUNDL:
			if (s7) {
				if (node->rodata->gundl.opagdl != NULL) {
					int base = (intptr_t)node->rodata->gundl.baseaddr;
...
bool projectileTryEmbedOnChr(struct chrdata *chr, struct coord *start, struct coord *end, struct coord *dir, float maxdist, struct coord *rayStart, struct coord *rayDir, struct coord *outPos, struct coord *outNormal, float *closestDist)
{
	...
	float x = (prop->pos.f[0] - start->f[0]);
	float y = (prop->pos.f[1] - start->f[1]);
	float z = (prop->pos.f[2] - start->f[2]);
	float spd4 = x * dir->f[0] + y * dir->f[1] + z * dir->f[2];
	int hitpart = 0;
	...
	if (-spe4 <= spd4 && spd4 <= maxdist + spe4 && utilsIsPointInCone(start, dir, &prop->pos, spe4)) {
		if ((prop->flags & PROPFLAG_ONTHISSCREENTHISTICK)) {
			if (g_ExtraBoundsDist > 0.0f) {
				hitpart = modelTestForHit(model, rayStart, rayDir, &spcc);

				while (hitpart > 0) {
					if (objTestAABBIntersection(model, spcc, rayStart, rayDir, &sp7c, &sp78, &sp74)) {
						mtx4TransformVec((Mtx*)&model->matrices[sp78], &sp7c.pos, &spb8);

						spec = (spb8.f[0] - rayStart->f[0]) * rayDir->f[0]
							+ (spb8.f[1] - rayStart->f[1]) * rayDir->f[1]
							+ (spb8.f[2] - rayStart->f[2]) * rayDir->f[2];

						if (spec < *closestDist) {
							mtx4RotateVec((Mtx*)&model->matrices[sp78], &sp7c.unk0c, &spac);

							*closestDist = spec;

							mtx4TransformVec(camGetProjectionMtx(), &spb8, outPos);
							mtx4RotateVec(camGetProjectionMtx(), &spac, outNormal);
				...
				if (hitpart > 0
						&& func0f06bea0(model, model->definition->rootnode, model->definition->rootnode, rayStart, rayDir, &sp7c.pos, &spec, &spcc, &hitpart, &sp78, &sp74)
						&& spec < *closestDist) {
					*closestDist = spec;
					mtx4TransformVec(camGetProjectionMtx(), &sp7c.pos, outPos);
					mtx4RotateVec(camGetProjectionMtx(), &sp7c.unk0c, outNormal);
		...
		} else if (projectileTryEmbedOnProp(prop, start, end, dir, outPos, outNormal, closestDist)) {
	...
	if (prop->flags & PROPFLAG_ONTHISSCREENTHISTICK) {
		...
		while (child) {
			if (child->flags & PROPFLAG_ONTHISSCREENTHISTICK) {
				if (projectileTestEmbedComplex(child->obj, start, end, dir, maxdist, rayStart, rayDir, outPos, outNormal, closestDist)) {
					result = true;
				}
			}
...
bool projectileFindCollidingProp(struct prop *prop, struct coord *pos1, struct coord *pos2, uint32_t cdtypes, struct coord *arg4, struct coord *arg5, RoomNum *rooms)
{
	...
	mtx4TransformVecInPlace(camGetPlayerWorldToScreenMtx(), &sp88);

	sp7c.x = sp98.x;
	sp7c.y = sp98.y;
	sp7c.z = sp98.z;

	mtx4RotateVecInPlace(camGetPlayerWorldToScreenMtx(), &sp7c);
		...
		for (propnumptr = propnums; *propnumptr >= 0; propnumptr++) {
			...
			if (iterprop != prop) {
				if (iterprop->type == PROPTYPE_OBJ
					...
					if (projectileTryEmbedOnChr(chr, pos1, pos2, &sp98, dist, &sp88, &sp7c, arg4, arg5, &spa8)) {
						spa4 = true;
					}
...
int func0f06cd00(struct defaultobj *obj, struct coord *pos, struct coord *arg2, struct coord *arg3)
{
	...
	if ((prop->pos.x != pos->x || prop->pos.y != pos->y || prop->pos.z != pos->z)
		...
		portalTraceLineThroughRooms(&prop->pos, &sp1c4, prop->rooms, spb8, spcc, 20);
		...
		for (i = 0; spcc[i] != -1; i++) {
			...
			if (bgRoomIsLoaded(spcc[i])) {
				if (bgTestHitInRoom(&prop->pos, &sp1c4, spcc[i], &hitthing, false)) {
					...
					g_HitTextureNum = hitthing.texturenum;
		...
		if (cdresult != CDRESULT_NOCOLLISION) {
			...
			if (arg3->x != 0.0f || arg3->y != 0.0f || arg3->z != 0.0f) {
				utilsNormalizeF(&arg3->x, &arg3->y, &arg3->z);
			} else {
				arg3->z = 1.0f;
			}
		}
		...
		if (obj->hidden & OBJHFLAG_PROJECTILE) {
			...
			if (!result) {
				...
				if (!sp98) {
					...
					propUpdatePositionRoomsSimple(&prop->pos, prop->rooms, &sp4c, rooms);
 ...
 */
void applySpeed(float *distdone, float maxdist, float *speedptr, float accel, float decel, float maxspeed)
{
	float speed = *speedptr;
	int i;

	for (i = 0; i < g_Vars.lvupdate60; i++) {
		float limit = speed * speed * 0.5f / decel;
		float distremaining = maxdist - *distdone;
 ...
 */
void projectileFall(struct defaultobj *obj, float arg1[3][3])
{
	int t2, t3, t4;
	struct coord sp188;
	Mtx sp148;
	...
	if (obj->hidden & OBJHFLAG_PROJECTILE) {
		...
		mtx4GetRotation(&sp148, &sp188);
		mtx4LoadRotation(&sp188, &sp108);
		quaternionEulerToQuat(&sp188, projectile->unk068);
		mtx4LoadRotationFrom(&sp108, &spc8);
		mtx4MultMtx4(&spc8, &sp148, &sp88);

		projectile->unk0b8[0] = sqrtf(sp88[0][0] * sp88[0][0] + sp88[0][1] * sp88[0][1] + sp88[0][2] * sp88[0][2]);
		projectile->unk0b8[1] = sqrtf(sp88[1][0] * sp88[1][0] + sp88[1][1] * sp88[1][1] + sp88[1][2] * sp88[1][2]);
		projectile->unk0b8[2] = sqrtf(sp88[2][0] * sp88[2][0] + sp88[2][1] * sp88[2][1] + sp88[2][2] * sp88[2][2]);
		...
		if (sp84 != 0.0f || sp80 != 0.0f) {
			float f0 = sqrtf(sp84 * sp84 + sp80 * sp80);
		...
		mtx4GetRotation((Mtx*)&spc8, &sp188);
		...
		sp6c = acosf(spc8[t2][0] * sp108[t2][0] + spc8[t2][1] * sp108[t2][1] + spc8[t2][2] * sp108[t2][2]);
...
void knifePlayWooshSound(struct defaultobj *obj)
{
	if (obj->hidden & OBJHFLAG_PROJECTILE) {
		if ((obj->projectile->flags & PROJECTILEFLAG_AIRBORNE)
			...
			uint16_t soundnums[] = { SFX_8074, SFX_8074, SFX_8074 };
			int index = rngRandom() % ARRAYCOUNT(soundnums);
...
void knifeLand(struct defaultobj *obj, struct coord *arg1, struct coord *arg2)
{
	...
	struct prop *prop = obj->prop;
	RoomNum newrooms[8];
	struct coord scatterDir;

	scatterDir.x = RANDOMFRAC() * 0.8f + arg2->x - 0.4f;
	scatterDir.y = RANDOMFRAC() * 0.8f + arg2->y - 0.4f;
	scatterDir.z = RANDOMFRAC() * 0.8f + arg2->z - 0.4f;

	mtxBuildRotationTowardsVector(&scatterDir, &lookMatrix);
	mtx4LoadXRotation(-M_PI_2, &pitchMatrix);
	mtx4MultMtx4(&lookMatrix, &pitchMatrix, &finalMatrix);
	mtxScaleRotationPart(obj->model->scale, &finalMatrix);
	...
	propUpdatePositionRoomsSimple(&prop->pos, prop->rooms, &newpos, newrooms);
	func0f06a580(obj, &newpos, &finalMatrix, newrooms);
}
...
bool objEmbed(struct prop *prop, struct prop *parent, struct model *model, struct modelnode *node)
{
	if (parent->flags & PROPFLAG_ONTHISSCREENTHISTICK) {
		...
		if (obj->embedment) {
			sp24 = (Mtx*)modelFindNodeMtx(model, node, 0);
			...
			mtxApplyAffineTransform(camGetProjectionMtx(), sp24, &spf4);
			mtxInvertAffine(&spf4, &spb4);
			mtxApplyAffineTransform(&spb4, &sp134, (Mtx*)&obj->embedment->matrix);
...
bool propExplode(struct prop *prop, int exptype)
{
	struct defaultobj *obj = prop->obj;
	int playernum = (obj->hidden & 0xf0000000) >> 28;
	bool result;
	...
	if (prop->parent) {
		...
		if (prop->flags & PROPFLAG_ONTHISSCREENTHISTICK) {
			...
			mtx4TransformVecInPlace(camGetProjectionMtx(), &pos);
		} else {
			pos.x = parent->pos.x;
			pos.y = parent->pos.y;
			pos.z = parent->pos.z;
		...
		propUpdatePositionRoomsSimple(&parent->pos, parent->rooms, &pos, rooms);
		...
		struct coord sp5c;
		struct coord sp50;
		float ymin = objGetLocalYMin(modelFindBboxRodata(obj->model));
		int room = prop->rooms[0];
	...
	if (((weapon->weaponnum == WEAPON_GRENADE && weapon->gunfunc == FUNC_PRIMARY)
		...
		if (weapon->timer240 >= 0) {
			...
			if (weapon->timer240 < 0) {
				...
				struct prop *ownerprop = NULL;
				int ownerplayernum = (obj->hidden & 0xf0000000) >> 28;
		...
		if (weapon->timer240 >= 2) {
			...
			int exptype = EXPLOSIONTYPE_ROCKET;
		...
		if (weapon->timer240 >= 2) {
			...
			float xdist = playerpos->f[0] - prop->pos.f[0];
			float ydist = playerpos->f[1] - prop->pos.f[1];
			float zdist = playerpos->f[2] - prop->pos.f[2];
		...
		if (weapon->timer240 >= 2) {
			// Bolt is travelling
			struct modelrodata_bbox *bbox = modelFindBboxRodata(obj->model);
			int ival = weapon->timer240 - 1;
			float radians = 0.026179939508438f * (ival / 12.0f);
			Mtx spf8;
			Mtx spb8;
			Mtx sp78;
			struct coord sp6c;
			...
			if (obj->embedment) {
				if (prop->parent && prop->parent->type != PROPTYPE_CHR) {
					mtx4Copy((Mtx*)&obj->embedment->matrix, &spf8);

					spf8[3][0] = spf8[3][1] = spf8[3][2] = 0.0f;
					spf8[0][3] = spf8[1][3] = spf8[2][3] = 0.0f;
					...
					sp78[3][0] = obj->embedment->matrix[3][0] - (sp60.f[0] - sp6c.f[0]);
					sp78[3][1] = obj->embedment->matrix[3][1] - (sp60.f[1] - sp6c.f[1]);
					sp78[3][2] = obj->embedment->matrix[3][2] - (sp60.f[2] - sp6c.f[2]);
...
void func0f07079c(struct prop *prop, bool fulltick)
{
	...
	if (model->attachedtonode && (obj->hidden & OBJHFLAG_EMBEDDED)) {
		Mtx *mtx = modelFindNodeMtx(model->attachedtomodel, model->attachedtonode, 0);
		struct modelrenderdata renderdata = {NULL, true, 3};
		Mtx sp30;

		prop->flags |= PROPFLAG_ONTHISSCREENTHISTICK | PROPFLAG_ONANYSCREENTHISTICK;
		mtxApplyAffineTransform(mtx, (Mtx*)&obj->embedment->matrix, &sp30);
...
int glassCalculateOpacity(struct coord *pos, float opadist, float maxopacity, float minopacity)
{
	struct coord *campos = &g_Vars.currentplayer->cam_pos;
    int opacity;
    float xdiff = pos->x - campos->x;
    float ydiff = pos->y - campos->y;
    float zdiff = pos->z - campos->z;
...
void func0f070a1c(struct modelrodata_bbox *bbox, float rot[3][3], struct coord *pos, struct coord *vertices)
{
	float sp54 = rot[0][0] * bbox->xmin;
	float sp50 = rot[0][1] * bbox->xmin;
	float sp4c = rot[0][2] * bbox->xmin;

	float sp48 = rot[2][0] * bbox->zmin;
	float sp44 = rot[2][1] * bbox->zmin;
	float sp40 = rot[2][2] * bbox->zmin;

	float sp3c = rot[0][0] * bbox->xmax;
	float sp38 = rot[0][1] * bbox->xmax;
	float sp34 = rot[0][2] * bbox->xmax;

	float sp30 = rot[2][0] * bbox->zmax;
	float sp2c = rot[2][1] * bbox->zmax;
	float sp28 = rot[2][2] * bbox->zmax;

	float sp24 = rot[1][0] * bbox->ymin + pos->f[0];
	float sp20 = rot[1][1] * bbox->ymin + pos->f[1];
	float sp1c = rot[1][2] * bbox->ymin + pos->f[2];
...
void func0f070bd0(struct modelrodata_type19 *rodata, float rot[3][3], struct coord *pos, struct coord *vertices)
{
	int i;
...
void liftUpdateTiles(struct liftobj *lift, bool stationary)
{
	...
	do {
		geo = (uint8_t *)lift->base.unkgeo + lift->base.geocount * 0x40;
		bbox = NULL;
		rodata = NULL;
...
void liftGoToStop(struct liftobj *lift, int stopnum)
{
	...
	if (lift->pads[stopnum] >= 0 && lift->levelaim != stopnum) {
				 ...
				 (curpad.pos.z >= aimpad.pos.z && curpad.pos.z >= reqpad.pos.z)))
		{
			...
			// Reverse direction
			float xdiff = aimpad.pos.x - curpad.pos.x;
			float ydiff = aimpad.pos.y - curpad.pos.y;
			float zdiff = aimpad.pos.z - curpad.pos.z;
			float result = sqrtf(xdiff * xdiff + ydiff * ydiff + zdiff * zdiff);
...
float objGetHovBobOffsetY(struct defaultobj *obj)
{
	...
	if (hov) {
		...
		struct modelrodata_bbox *bbox = objFindBboxRodata(obj);
		float value = objGetRotatedLocalYMinByMtx3(bbox, obj->realrot);
		result = objGetPlacementYOffset(obj) - value;
	}
...
void hovTick(struct defaultobj *obj, struct hov *hov)
{
	...
	if (obj->flags & OBJFLAG_DEACTIVATED) {
		...
		propUpdatePositionRoomsSimple(&prop->pos, prop->rooms, &sp1b4, sp198);
		roomsAppend(sp9c, sp198, ARRAYCOUNT(sp198));
		ground1 = cdFindGroundAtCyl(&sp1b4, 5, sp198, &obj->floorcol, NULL);

		propUpdatePositionRoomsSimple(&prop->pos, prop->rooms, &sp1a8, sp188);
	...
	for (i = 0; i < g_Vars.lvupdate60; i++) {
		...
		hov->groundpitch += (groundangle - hov->groundpitch) * 0.075f;
	...
	mtxApplyAffineTransformInPlace(&sp108, &sp148);
	mtx4LoadYRotation(hov->yrot, &sp108);
	mtxApplyAffineTransformInPlace(&sp108, &sp148);
	mtxScaleRotationPart(obj->model->scale, &sp148);

	if (obj->type == OBJTYPE_HOVERBIKE) {
		struct hoverbikeobj *bike = (struct hoverbikeobj *) obj;
		float ezreal = bike->ezreal + bike->ezreal2;
...
int func0f072144(struct defaultobj *obj, struct coord *arg1, float arg2, bool arg3)
{
	...
	if (objIsHoverpropOrBike(obj)) {
		...
		mtxScaleRotationPart(obj->model->scale, &spa4);
		...
		if (arg2 != 0.0f) {
			...
			mtx3x3TransposeMul(sp40, obj->realrot, sp460);
		} else {
	...
	if (cdresult == CDRESULT_NOCOLLISION && arg3) {
		...
		if (obj->geocyl && (obj->hidden2 & OBJH2FLAG_DYNAMIC_COLLISION)) {
			if (obj->flags3 & OBJFLAG3_GEOCYL) {
...
{
	int index = hovercar->path - g_StageSetup.paths + 1;
 ...
 */
float objCollide(struct defaultobj *movingobj, struct coord *movingvel, float rotation)
{
	float force = 1.0f;
	...
	if (obstacle && g_Vars.lvupdate240 > 0) {
		if (obstacle->type == PROPTYPE_CHR || obstacle->type == PROPTYPE_PLAYER) {
			...
			if ((obstacleobj->hidden & OBJHFLAG_MOUNTED) == 0
				...
				if (rotation != 0.0f) {
					float xdiff = sp88.f[0] - movingobj->prop->pos.f[0];
					float zdiff = sp88.f[2] - movingobj->prop->pos.f[2];
					float finalrotation = 0.0f;
					struct coord speed = {0, 0, 0};
					float mult = 1.0f / sqrtf(xdiff * xdiff + zdiff * zdiff);
...
void hoverbikeUpdateMovement(struct hoverbikeobj *bike, float speedforwards, float speedsideways, float speedtheta)
{
	float f12;
	float angle;
	float sinangle;
...
bool rocketTickFbw(struct weaponobj *rocket)
{
	...
	if (ownerchr) {
		...
		for (i = 0; i < g_Vars.lvupdate240; i++) {
			projectile->unk018 = modelTweenRotAxis(projectile->unk018, xrot, 0.01875f);
	...
	if (cdresult == CDRESULT_NOCOLLISION) {
		propUpdatePositionRoomsSimple(&rocketprop->pos, rocketprop->rooms, &newpos, newrooms);
...
int projectileLaunch(struct defaultobj *obj, struct projectile *projectile, struct coord *arg2, struct coord *arg3)
{
	...
	if (cdresult == CDRESULT_NOCOLLISION) {
		...
		propUpdatePositionRoomsSimple(&prop->pos, prop->rooms, &projectile->nextsteppos, rooms);
		...
		if (weapon->weaponnum == WEAPON_ROCKET || weapon->weaponnum == WEAPON_HOMINGROCKET) {
			...
			propUpdatePositionRoomsSimple(&prop->pos, prop->rooms, arg2, rooms);
	...
	if (obj->type == OBJTYPE_WEAPON && ((struct weaponobj *)obj)->weaponnum == WEAPON_SKROCKET) {
		...
		if (projectile->flags & PROJECTILEFLAG_SLIDING) {
			...
			if ((projectile->speed.f[0] != 0.0f || projectile->speed.f[2] != 0.0f) && projectile->unk098 > 0.0f) {
				...
				if (dist > 0.0f) {
					float f12 = projectile->unk098 * g_Vars.lvupdate60freal / dist;
			...
			if (cdresult == CDRESULT_COLLISION) {
				...
				if (sp58c > 0.0f) {
					...
					if (sp3f4.f[0] != 0.0f || sp3f4.f[2] != 0.0f) {
						utilsNormalizeF(&sp3f4.x, &sp3f4.y, &sp3f4.z);
					} else {
						sp3f4.z = 1.0f;
					}
			...
			if (homingrocket) {
				...
				if (obj && obj->prop && projectile->targetprop) {
					Mtx mtx;
					float sp29c = sqrtf(obj->realrot[0][0] * obj->realrot[0][0] + obj->realrot[1][0] * obj->realrot[1][0] + obj->realrot[2][0] * obj->realrot[2][0]);
					struct coord sp290;
					float sp28c;

					mtxIdent(&mtx);

					mtx[0][0] = obj->realrot[0][0] / sp29c;
					mtx[0][1] = obj->realrot[0][1] / sp29c;
					mtx[0][2] = obj->realrot[0][2] / sp29c;
					mtx[1][0] = obj->realrot[1][0] / sp29c;
					mtx[1][1] = obj->realrot[1][1] / sp29c;
					mtx[1][2] = obj->realrot[1][2] / sp29c;
					mtx[2][0] = obj->realrot[2][0] / sp29c;
					mtx[2][1] = obj->realrot[2][1] / sp29c;
					mtx[2][2] = obj->realrot[2][2] / sp29c;
					...
					utilsNormalizeF(&sp290.x, &sp290.y, &sp290.z);

					sp2ec.x = projectile->speed.x;
					sp2ec.y = projectile->speed.y;
					sp2ec.z = projectile->speed.z;

					utilsNormalizeF(&sp2ec.x, &sp2ec.y, &sp2ec.z);
					...
					if (sp28c > 0.001f || sp28c < -0.001f) {
						...
						static float var80069bc4 = 0;
						static uint32_t kkg = 3;
						static uint32_t kkd = 20;
						static uint32_t kkp = 120;
						...
						quaternion3x3MtxToQuat((Mtx*)&mtx, sp270);
						quaternionMultQuaternion(sp270, sp260, sp250);
						quaternionToMtx(sp250, &sp20c);

						obj->realrot[0][0] = sp20c[0][0] * sp29c;
						obj->realrot[0][1] = sp20c[0][1] * sp29c;
						obj->realrot[0][2] = sp20c[0][2] * sp29c;
						obj->realrot[1][0] = sp20c[1][0] * sp29c;
						obj->realrot[1][1] = sp20c[1][1] * sp29c;
						obj->realrot[1][2] = sp20c[1][2] * sp29c;
						obj->realrot[2][0] = sp20c[2][0] * sp29c;
						obj->realrot[2][1] = sp20c[2][1] * sp29c;
						obj->realrot[2][2] = sp20c[2][2] * sp29c;
					}
				}
			}
			...
			if (projectile->flags & PROJECTILEFLAG_STICKY) {
				...
				cdresult = projectileTryMove(obj, &sp5dc, &sp5e8, &sp5f4);
			}
			...
			if (projectile->flags & PROJECTILEFLAG_STICKY) {
					...
					if (!handled && g_EmbedProp && obj->type == OBJTYPE_WEAPON) {
						...
						if (weapon->weaponnum == WEAPON_BOLT || weapon->weaponnum == WEAPON_COMBATKNIFE) {
							if (hitprop->type == PROPTYPE_CHR || (hitprop->type == PROPTYPE_PLAYER && hitprop->chr)) {
									...
									if (ownershield <= 0.0f) {
										...
										if (g_EmbedProp->flags & PROPFLAG_ONTHISSCREENTHISTICK) {
											if (g_EmbedModel && g_EmbedHitPart != HITPART_GUN && g_EmbedHitPart != HITPART_HAT) {
												...
												mtxInvertRigidBodyMatrix(sp184, &sp188);
								...
								if (hitobj->modelnum == MODEL_TARGET && g_HitTextureNum == TEXTURE_0B9E) {
							...
							if (g_EmbedProp->type == PROPTYPE_CHR || (g_EmbedProp->type == PROPTYPE_PLAYER && g_EmbedProp->chr)) {
								...
								if (g_HitTextureNum == 10000) {
									float shield = (g_EmbedProp->obj->flags3 & OBJFLAG3_SHOWSHIELD) ? 4 : 8;
							...
							} else if ((hitprop->type == PROPTYPE_OBJ || hitprop->type == PROPTYPE_WEAPON) && g_HitTextureNum == 10000) {
						...
						if (obj->type == OBJTYPE_WEAPON) {
							...
							if (weapon->weaponnum == WEAPON_BOLT || weapon->weaponnum == WEAPON_COMBATKNIFE) {
								...
								if (hitprop == NULL || hitprop->type == PROPTYPE_OBJ || hitprop->type == PROPTYPE_WEAPON || hitprop->type == PROPTYPE_DOOR) {
									...
									utilsNormalizeF(&dir.x, &dir.y, &dir.z);
				...
				if (!handled) {
					if (cdresult != CDRESULT_COLLISION) {
						...
						propUpdatePositionRoomsSimple(&prop->pos, prop->rooms, &sp5dc, rooms);
			...
			if (!handled) {
						...
						&& !cdTestLos03(&sp5c8, sp5b8, &sp5ac, CDTYPE_OBJS | CDTYPE_BG, GEOFLAG_FLOOR1 | GEOFLAG_FLOOR2))
				{
					...
					utilsNormalizeF(&sp5f4.x, &sp5f4.y, &sp5f4.z);
				...
				if (cdresult == CDRESULT_COLLISION) {
					...
					if (projectile->bounciness > 0.0f) {
						float oldyspeed;
						float f0 = projectile->speed.f[0] * sp5f4.f[0] + projectile->speed.f[1] * sp5f4.f[1] + projectile->speed.f[2] * sp5f4.f[2];
				...
				if (obj->type == OBJTYPE_WEAPON) {
					...
					if (weapon->weaponnum == WEAPON_COMBATKNIFE && weapon->gunfunc == FUNC_SECONDARY) {
						...
						if (cdresult == CDRESULT_COLLISION) {
							...
							float tmp = projectile->speed.f[0] * projectile->speed.f[0]
							...
							if (projectile->powerlimit240 >= 0 && projectile->flighttime240 > projectile->powerlimit240) {
								...
								utilsNormalizeF(&smokepos.x, &smokepos.y, &smokepos.z);
					...
					if (cdresult == CDRESULT_COLLISION) {
						if (projectile->unk0a4 < g_Vars.lvframenum - 2) {
							if (weapon->weaponnum == WEAPON_COMBATKNIFE || weapon->weaponnum == WEAPON_COMBATKNIFE) {
								...
								uint16_t sp100[] = {SFX_0027, SFX_0028, SFX_0029, SFX_002A};
			...
			if (projectile->unk060 < 1.0f) {
				...
				mtxScaleRow0Vec(projectile->unk0b8[0], &spac);
				mtxScaleRow1Vec(projectile->unk0b8[1], &spac);
				mtxScaleRow2Vec(projectile->unk0b8[2], &spac);
			...
			if (projectile->speed.f[0] != 0.0f || projectile->speed.f[2] != 0.0f || projectile->unk060 < 1.0f) {
				...
				float spa4;
				RoomNum roomnum;
				float sp98 = objGetRotatedLocalYMinByMtx3(objFindBboxRodata(obj), obj->realrot);
				uint16_t geoflags;

				stop = false;

				for (int i = 0; i < g_Vars.lvupdate60; i++) {
					sp5dc.x += projectile->speed.x;
					sp5dc.z += projectile->speed.z;

					if (projectile->unk060 >= 1.0f) {
						if (projectile->unk098 > 0.0f) {
							float dist = sqrtf(projectile->speed.f[0] * projectile->speed.f[0] + projectile->speed.f[2] * projectile->speed.f[2]);
				...
				roomsCopy(prop->rooms, sp5b8);
				projectileTryMove(obj, &sp5dc, &sp5e8, &sp5f4);
				...
				if (roomnum > 0) {
					prop->pos.y = spa4 - sp98 + objGetPlacementYOffset(obj);
...
void doorTick(struct prop *doorprop)
{
	...
	float prevfrac = door->frac;

	static uint32_t debugdoor = 0;

	// If debugdoor is set to 1 or to the address of this door,
	// print the distance to the door to console
	if (debugdoor) {
		uint32_t addr = (uint32_t)doorprop;

		if (debugdoor == 1 || debugdoor == addr) {
			float xdiff = doorprop->pos.x - g_Vars.players[0]->cam_pos.x;
			float zdiff = doorprop->pos.z - g_Vars.players[0]->cam_pos.z;
	...
	if (model->definition->skel == &g_Skel13) {
		float soundpoint = door->maxfrac * 0.3f;
...
void doorUpdatePortalIfWindowed(struct prop *doorprop, int playercount)
{
...
#define MTX(i) ((Mtx *)((uintptr_t)matrices + i * sizeof(Mtx)))
...
void doorInitMatrices(struct prop *prop)
{
	...
	mtxApplyAffineTransformInPlace(camGetPlayerWorldToScreenMtx(), matrices);

	if (model->definition->skel == &g_Skel11) {
		union modelrodata *rodata;
		float xrot = M_TAU - door->frac * 0.017450513318181f;

		rodata = modelGetPartRodata(model->definition, MODELPART_0001);
		mtx4LoadXRotation(xrot, (Mtx*)MTX(1));
		mtx4SetTranslation(&rodata->position.pos, (Mtx*)MTX(1));
		mtx4MultMtx4InPlace((Mtx*)MTX(0), (Mtx*)MTX(1));

		rodata = modelGetPartRodata(model->definition, MODELPART_0002);
		mtx4LoadXRotation(M_TAU - xrot, (Mtx*)MTX(2));
		mtx4SetTranslation(&rodata->position.pos, (Mtx*)MTX(2));
		mtx4MultMtx4InPlace((Mtx*)MTX(0), (Mtx*)MTX(2));
	} else if (model->definition->skel == &g_Skel13) {
		union modelrodata *rodata;
		float zrot1 = 0;
		float zrot2 = door->frac * 0.017450513318181f;
		float limit = door->maxfrac * 0.3f;
		int i;
		...
		for (i = 0; i < 6; i++) {
			...
			mtx4LoadZRotation(zrot1, (Mtx*)MTX(index1));
			mtx4SetTranslation(&rodata->position.pos, (Mtx*)MTX(index1));
			mtx4MultMtx4InPlace((Mtx*)MTX(0), (Mtx*)MTX(index1));

			rodata = modelGetPartRodata(model->definition, index2);
			mtx4LoadZRotation(zrot2, (Mtx*)MTX(index2));
			mtx4SetTranslation(&rodata->position.pos, (Mtx*)MTX(index2));
			mtx4MultMtx4InPlace((Mtx*)MTX(index1), (Mtx*)MTX(index2));
		}
	}
}
			...
			if (platformobj->type == OBJTYPE_LIFT) {
				if (g_Vars.players[playernum]->lift == platform && g_Vars.players[playernum]->bondmovemode == MOVEMODE_WALK) {
					...
					if (g_Vars.players[playernum]->inlift && !g_Vars.players[playernum]->onladder && !g_Vars.players[playernum]->isfalling) {
						struct coord sp78;
						RoomNum sp68[8];
						float ydist = newpos->y - prevpos->y;
						...
						if (ydist != 0.0f) {
							...
							if (ydist > 0.0f || (platformobj->flags & OBJFLAG_LIFT_CHECKCEILING) == 0) {
								...
								propUpdatePositionRoomsSimple(&prop->pos, prop->rooms, &sp78, sp68);
								...
								g_Vars.players[playernum]->sumground = g_Vars.players[playernum]->vv_manground / 0.045499980449677f;
...
void escastepTick(struct prop *prop)
{
	...
	for (i = 0; true; i++) {
		if (keyframes[i + 1].frame == -1) {
			...
			mult = (float)(step->frame - keyframes[i].frame) / (float)(keyframes[i + 1].frame - keyframes[i].frame);
...
void cctvTick(struct prop *camprop)
{
	...
	if (canseeplayer) {
		float angle = atan2f(xdist, zdist);
	...
	if (canseeplayer) {
		float angle = atan2f(ydist, sqrtf(xdist * xdist + zdist * zdist));
		float finalangle = angle - camera->xzero;
	...
	if (canseeplayer) {
		...
		if (camera->seebondtime60 >= (int)(TICKS(300) * g_CctvWaitScale)) {
			alarmActivate();
	...
	// Update yaw
	if (camera->yrot < yaw) {
		float tmp = camera->yspeed * camera->yspeed * 764.06536865234f;
		...
		if (camera->yrot >= yaw - tmp) {
			...
			float newspeed = camera->yspeed + 0.00065439427271485f * g_Vars.lvupdate60freal;
...
void cctvInitMatrices(struct prop *prop, Mtx *mtx)
{
	...
	mtxApplyAffineTransformInPlace(camGetPlayerWorldToScreenMtx(), &matrices[1]);
}
...
void fanUpdateModel(struct prop *prop)
{
	...
	float sp48[3][3];
	float sp24[3][3];
	float angle = fan->yspeed * g_Vars.lvupdate60freal;

	while (angle >= M_TAU) {
		angle -= M_TAU;
	}

	mtx4LoadYRotation(angle, &fanTransformMtx);
	mtx4ToMtx3(&fanTransformMtx, sp48);
	mtx3x3TransposeMul(fan->base.realrot, sp48, sp24);
...
void autogunTick(struct prop *prop)
{
	...
	if (obj->flags2 & OBJFLAG2_AUTOGUN_MALFUNCTIONING1) {
		...
		applyRotation(&autogun->yrot, autogun->yzero, &autogun->yspeed,
			0.00001163367596746f,
			0.00001163367596746f,
			0.00069802056532353f);
		applyRotation(&autogun->xrot, autogun->xzero, &autogun->xspeed,
			0.0000058168379837298f,
			0.0000058168379837298f,
			0.00034901028266177f);
		return;
	}
...
void autogunInitMatrices(struct prop *prop, Mtx *mtx)
{
	...
	union modelrodata *rodata;
	struct coord sp4c;
	float yrot = autogun->yrot + M_PI_2;
	float xrot = -autogun->xrot;
	Mtx *tmp;
	struct modelnode *node2;
	...
	mtxScaleRotationPart(autogun->base.model->scale, &matrices[1]);
	mtxApplyAffineTransformInPlace(camGetPlayerWorldToScreenMtx(), &matrices[1]);
	...
	mtxApplyAffineTransformInPlace(&matrices[1], &matrices[2]);
	...
	if (tmp != NULL) {
		...
		mtxApplyAffineTransformInPlace(&matrices[1], tmp);
	}
	...
	if (node3 != NULL) {
		...
		mtx4SetTranslation(&rodata->position.pos, tmp);
		mtxApplyAffineTransformInPlace(&matrices[2], tmp);
	}
		...
		if (autogun->firing && (obj->flags & OBJFLAG_DEACTIVATED) == 0) {
			...
			if (fireleft || fireright) {
				...
				if ((autogunprop->flags & PROPFLAG_ONTHISSCREENTHISTICK) && (flashnode || posnode)) {
					...
					mtxApplyAffineTransform(camGetProjectionMtx(), sp108, &spc8);
				...
				if (g_Vars.normmplayerisrunning
					...
					if (g_Vars.lvframe60 == autogun->lastaimbond60
						...
						float y;
						float z;
						float sqguntotargetdist;
						float sqguntohitdist;
						float damage;
						...
						if (sqguntohitdist >= sqguntotargetdist) {
							float guntotargetdist = sqrtf(sqguntotargetdist);
							float increment = 0.16f * g_Vars.lvupdate60freal * g_AutogunAccuracyScale;
				...
				if (missed) {
					portalTraceLineThroughRooms(&gunpos, &hitpos, gunrooms, hitrooms, NULL, 0);
...
bool chopperCheckTargetInFov(struct chopperobj *hovercar, uint8_t fov)
{
	...
	if (chopper) {
		...
		float angle = atan2f(prop->pos.x - target->pos.x, prop->pos.z - target->pos.z);
		float anglediff = angle - roty;
...
float chopperGetTimer(struct chopperobj *obj)
{
	struct chopperobj *chopper = chopperFromHovercar(obj);
...
void chopperSetMaxDamage(struct chopperobj *chopper, uint16_t health)
{
...
float func0f07b164(struct coord *pos1, struct coord *pos2, struct coord *pos3, struct coord *result)
{
	...
	utilsNormalizeF(&sp34.x, &sp34.y, &sp34.z);
...
void chopperFireRocket(struct chopperobj *chopper, bool side)
{
	if (chopper->ontarget) {
		...
		Mtx rocketOffsetMtx;
		Mtx chopperRotationMtx;

		// Rocket launcher position relative to chopper center
		pos.x = side ? -754 : 754;
		pos.y = -400;
		pos.z = -400;

		// Build transform for the rocket launcher
		mtx3ToMtx4(chopper->base.realrot, &chopperRotationMtx);
		mtx4LoadTranslation(&pos, &rocketOffsetMtx);
		mtx4MultMtx4InPlace(&chopperRotationMtx, &rocketOffsetMtx);

		// Compute world position of rocket origin
		pos.x = rocketOffsetMtx[3][0] + chopperprop->pos.f[0];
		pos.y = rocketOffsetMtx[3][1] + chopperprop->pos.f[1];
		pos.z = rocketOffsetMtx[3][2] + chopperprop->pos.f[2];

		// Aim at target with slight vertical randomness
		direction.x = targetprop->pos.x - pos.x;
		direction.y = targetprop->pos.y - pos.y + (int)(rngRandom() % 100);
		direction.z = targetprop->pos.z - pos.z;

		utilsNormalizeF(&direction.x, &direction.y, &direction.z);
...
void chopperIncrementBarrel(struct prop *chopperprop, bool firing)
{
	...
	struct coord rot;
	struct coord gunpos;
	float gunroty = chopper->gunroty;
	float gunrotx = chopper->gunrotx;
	float gunturnyspeed60 = chopper->gunturnyspeed60;
	float gunturnxspeed60 = chopper->gunturnxspeed60;
	struct prop *targetprop = chopperGetTargetProp(chopper);
	struct modelnode *node;
	...
	if (firing) {
		...
		if (obj->modelnum == MODEL_A51INTERCEPTOR) {
			float scale = 0.1f / obj->model->scale;
			gunpos.x *= scale;
			gunpos.y *= scale;
			gunpos.z *= scale;
		}

		mtx3ToMtx4(obj->realrot, (Mtx*)&sp50);
		mtx4LoadTranslation(&gunpos, &sp90);
		mtx4MultMtx4InPlace(&sp50, &sp90);

		gunpos.x = sp90[3][0] + chopperprop->pos.f[0];
		gunpos.y = sp90[3][1] + chopperprop->pos.f[1];
		gunpos.z = sp90[3][2] + chopperprop->pos.f[2];
	...
	if (firing) {
		float totalrotx = chopper->gunrotx + chopper->rotx;
		float totalroty = chopper->gunroty + chopper->roty;
...
void chopperIncrementMovement(struct prop *prop, float goalroty, float goalrotx, struct coord *dir, bool firing)
{
	...
	if (chopper->attackmode != CHOPPERMODE_FALL) {
		...
		if (tmp > speed * speed) {
			utilsNormalizeF(&chopper->vx, &chopper->vy, &chopper->vz);
	...
	mtxScaleRotationPart(chopper->base.model->scale, &sp7c);
...
void chopperTickFall(struct prop *chopperprop)
{
	...
	struct chopperobj *chopper = (struct chopperobj *) obj;
	float goalroty;
	float *x = &chopper->fall.x;
	float *y = &chopper->fall.y;
	float *z = &chopper->fall.z;
	struct coord speed;
	int i;
	struct pad prevpad;
	struct pad nextpad;
	...
	if (stageGetIndex(g_Vars.stagenum) == STAGEINDEX_EXTRACTION) {
		...
		for (int i = 1; i < g_Vars.lvupdate60; i++) {
			mult *= 1.0f - 0.02f;
		}
		...
		if (cdExamLos09(&chopperprop->pos, chopperprop->rooms, &newpos, CDTYPE_BG) == CDRESULT_COLLISION) {
			...
			propUpdatePositionRoomsSimple(&chopperprop->pos, chopperprop->rooms, &newpos, newrooms);
...
void chopperTickCombat(struct prop *chopperprop)
{
	...
	if ((chopper->targetvisible && dist < 2000000.0f) || chopper->path == NULL) {
		...
		if (sp8c != sp90) {
			...
			utilsNormalizeF(&sp78.x, &sp78.y, &sp78.z);
...
void hovercarTick(struct prop *prop)
{
	...
	float sp200 = hovercar->roty;
	float sp1fc = hovercar->rotx;
	uint32_t ishoverbot = obj->flags & OBJFLAG_HOVERCAR_ISHOVERBOT;
	float sp1f4 = ishoverbot ? 5 : 10;
	...
	if (hovercar->dead) {
		if (ishoverbot) {
			...
			if (hovercar->speedtime60 == 0) {
				...
				propUpdatePositionRoomsSimple(&prop->pos, prop->rooms, &sp1c0, sp1b0);
	...
	if (hovercar->speed > 0)
	{
		...
		mtxScaleRotationPart(hovercar->base.model->scale, &spac);
		...
		propUpdatePositionRoomsSimple(&prop->pos, prop->rooms, &sp150, sp140);
...
void glassUpdatePortal(struct prop *prop, int playercount, bool *arg2)
{
...
void objInitMatrices(struct prop *prop)
{
	...
	if (obj->type == OBJTYPE_DOOR) {
		...
		mtxApplyAffineTransform(camGetPlayerWorldToScreenMtx(), &mtx, (Mtx*)obj->model->matrices);
		...
		if (obj->type == OBJTYPE_CCTV) {
			...
			if (obj->model->definition->nummatrices >= 2) {
				...
				mtx4Copy((Mtx*)obj->model->matrices, &sp28);
	...
	if (prop->timetoregen > 0) {
		...
		if (prop->timetoregen <= 0) {
			...
			if (obj->damage == 0 && (obj->hidden2 & OBJH2FLAG_DESTROYED) == 0) {
				...
				if (obj->hidden2 & OBJH2FLAG_DYNAMIC_COLLISION) {
					obj->flags |= OBJFLAG_GEOMETRY_TEMP_ENABLED;
				} else {
					obj->flags &= ~OBJFLAG_GEOMETRY_TEMP_ENABLED;
				}
...
int objTickPlayer(struct prop *prop)
{
	...
	bool fulltick = false;
	uint32_t playercount = PLAYERCOUNT();
	bool sp564 = true;
	bool embedded = false;
	bool sp556 = false;
		...
		if (g_Anims[model->anim->animnum].flags & ANIMFLAG_ABSOLUTETRANSLATION) {
			if (g_Vars.tickmode != TICKMODE_CUTSCENE
				...
				if ((g_Anims[model->anim->animnum].flags & ANIMFLAG_HASREPEATFRAMES)
					...
					sp556 = true;
					sp476.unk10 = gfxAllocate(model->definition->nummatrices * sizeof(Mtx));
					sp476.unk00 = (Mtx*)camGetPlayerWorldToScreenMtx();
					modelSetMatricesWithAnim(&sp476, model);

					if (fulltick) {
						mtxApplyAffineTransform(camGetProjectionMtx(), (Mtx*)model->matrices, &sp412);
						...
						if (obj->flags3 & OBJFLAG3_00000010) {
							propResolveNewPositionRooms(prop->rooms, &sp400, sp384);
						} else {
							propUpdatePositionRoomsSimple(&prop->pos, prop->rooms, &sp400, sp384);
						}
						...
						if (hov) {
							hovUpdateGround(obj, hov, &prop->pos, prop->rooms, obj->realrot);
							hoverpropSetTurnAngle(obj, atan2f(sp412[2][0], sp412[2][2]));
			...
			mtx4MultMtx4(camGetPlayerWorldToScreenMtx(), &sp248, &sp152);

			sp556 = true;
			sp312.unk10 = gfxAllocate(model->definition->nummatrices * sizeof(Mtx));
			sp312.unk00 = &sp152;
			modelSetMatricesWithAnim(&sp312, model);

			if (fulltick) {
				sp236.x = (float)sp116.x + prop->pos.x;
				sp236.y = prop->pos.y;
				sp236.z = (float)sp116.z + prop->pos.z;

				if (obj->flags3 & OBJFLAG3_00000010) {
					propResolveNewPositionRooms(prop->rooms, &sp236, sp220);
				} else {
					propUpdatePositionRoomsSimple(&prop->pos, prop->rooms, &sp236, sp220);
				}

				if (modelGetCurAnimFrame(model) >= modelGetNumAnimFrames(model) - 1) {
					modelmgrFreeAnim(model->anim);
					model->anim = NULL;
					mtxApplyAffineTransform(camGetProjectionMtx(), (Mtx*)model->matrices, &sp248);
				...
				if (sp148 <= sp144) {
					prop->pos.y = cdFindGroundAtCyl(&prop->pos, 5, prop->rooms, &obj->floorcol, NULL)
						+ objGetPlacementYOffset(obj) + sp112;
				}

				func0f069c70(obj, true, true);
				sp592 = true;

				if (objUpdateGeometry(prop, (uint8_t **)geos, &end)
	...
	while (prop) {
		if (prop->type == PROPTYPE_CHR) {
			...
			if (CHRRACE(chr) == RACE_ROBOT) {
				gdl = beamRender(gdl, chr->roboguns[0]->beam, true, true);
				gdl = beamRender(gdl, chr->roboguns[1]->beam, true, true);
			} else {
...
static inline void tvscreenWrapTexCoord(int *s0, int *s1, int *s2, int *s3)
{
	...
	if (node && (node->type & 0xff) == MODELNODETYPE_DL) {
		...
		while (!yielding) {
			...
			case TVCMD_SETCMDLIST:
				tvscreenSetCmdlist(screen, (uint32_t *) cmd->arg1);
				break;
			...
			case TVCMD_SETCOLOUR:
				...
				screen->rednew = ((uint32_t)cmd->arg1 >> 24) & 0xff;

				screen->greenold = screen->green;
				screen->greennew = ((uint32_t)cmd->arg1 >> 16) & 0xff;

				screen->blueold = screen->blue;
				screen->bluenew = ((uint32_t)cmd->arg1 >> 8) & 0xff;
		...
		if (screen->colinc > 0.0f) {
			...
			if (screen->colfrac < 1.0f) {
				screen->red = screen->redold + (int) ((screen->rednew - screen->redold) * screen->colfrac);
				screen->green = screen->greenold + (int) ((screen->greennew - screen->greenold) * screen->colfrac);
				screen->blue = screen->blueold + (int) ((screen->bluenew - screen->blueold) * screen->colfrac);
				screen->alpha = screen->alphaold + (int) ((screen->alphanew - screen->alphaold) * screen->colfrac);
			} else {
				screen->colfrac = 1.0f;
				screen->colinc = 0.0f;
		...
		if ((uint32_t)screen->tconfig < 100) {
			tconfig = &g_TexScreenConfigs[(int)screen->tconfig];
		} else {
		...
		if (tconfig != NULL) {
			...
			int s0 = tconfig->width * (screen->xmid + f20) * 32.0f;
			int t0 = tconfig->height * (screen->ymid + f24) * 32.0f;
			int s1 = tconfig->width * (screen->xmid - f14) * 32.0f;
			int t1 = tconfig->height * (screen->ymid + f16) * 32.0f;
			int s2 = tconfig->width * (screen->xmid - f20) * 32.0f;
			int t2 = tconfig->height * (screen->ymid - f24) * 32.0f;
			int s3 = tconfig->width * (screen->xmid + f14) * 32.0f;
			int t3 = tconfig->height * (screen->ymid - f16) * 32.0f;
...
void objRenderProp(struct prop *prop, struct modelrenderdata *renderdata, bool xlupass)
{
	if (prop->flags & PROPFLAG_ONTHISSCREENTHISTICK) {
		...
		if (obj->type == OBJTYPE_SINGLEMONITOR) {
			if (renderdata->flags & 1) {
				...
				if (obj->flags2 & OBJFLAG2_DRAWONTOP) {
					...
					sp60 = wallhitCalcLiftDistance(&prop->pos);
				} else {
					sp60 = 1;
				}
		...
		if (obj->type == OBJTYPE_DOOR) {
			...
			int level = objGetDestroyedLevel(obj);

			renderdata->cullmode = CULLMODE_NONE;

			if (renderdata->unk30 == 9) {
				int alpha = 100 + level * 50;
		...
		if (xlupass) {
			if (orthogonal) {
				player0f0c3320((Mtx*)model->matrices, model->definition->nummatrices);
			}
...
Gfx *gfxRenderRadialShadow(Gfx *gdl, float x, float y, float z, float angle, float radius, uint32_t colour)
{
	...
	mtx4MultMtx4(camGetPlayerWorldToScreenMtx(), &spc0, &sp80);
	mtx4Copy(&sp80, mtx);
...
Gfx *objRenderShadow(struct defaultobj *obj, Gfx *gdl)
{
	...
	int room = cdFindFloorRoomYColourFlagsAtPos(&obj->prop->pos, obj->prop->rooms, &y, NULL, NULL);
...
Gfx *objRender(struct prop *prop, Gfx *gdl, bool xlupass)
{
	...
	if (g_Vars.lvmpbotlevel && obj->type == OBJTYPE_WEAPON) {
		if (obj->flags3 & OBJFLAG3_HARDFREEING) {
			weapon = (struct weaponobj *)obj;
			alpha = ((float)alpha * (float)weapon->fadeouttimer60) * 0.016666667535901f;
				...
				if (ok) {
					...
					for (i = 0; i < rodata->numvertices; i++) {
...

						{
							...
							if ((int)(random2() % 100) < chance) {
								rwdata->vertices[i].colour = 0;
							}
						}

						rwdata->vertices[i].x += (int)(((int)(random2() % 20) - 10) * spb0[0]);
						rwdata->vertices[i].y += (int)(((int)(random2() % 20) - 10) * spb0[1]);
						rwdata->vertices[i].z += (int)(((int)(random2() % 20) - 10) * spb0[2]);

						if (parentbbox != NULL) {
							if (rwdata->vertices[i].x < (int16_t)parentbbox->xmin) {
								rwdata->vertices[i].x = (int16_t)parentbbox->xmin;
							}

							if (rwdata->vertices[i].x > (int16_t)parentbbox->xmax) {
								rwdata->vertices[i].x = (int16_t)parentbbox->xmax;
							}

							if (rwdata->vertices[i].y < (int16_t)parentbbox->ymin) {
								rwdata->vertices[i].y = (int16_t)parentbbox->ymin;
							}

							if (rwdata->vertices[i].y > (int16_t)parentbbox->ymax) {
								rwdata->vertices[i].y = (int16_t)parentbbox->ymax;
							}

							if (rwdata->vertices[i].z < (int16_t)parentbbox->zmin) {
								rwdata->vertices[i].z = (int16_t)parentbbox->zmin;
							}

							if (rwdata->vertices[i].z > (int16_t)parentbbox->zmax) {
								rwdata->vertices[i].z = (int16_t)parentbbox->zmax;
							}
						}
					}
				}
...
void objBounce(struct defaultobj *obj, struct coord *gundir2d)
{
	...
	if (projectile) {
		...
		mtx4RotateVecInPlace(camGetProjectionMtx(), &dir);
...
{
	struct coord speed = {0, 0, 0};
	float a = tween->f[0] * dir->f[0] + tween->f[2] * dir->f[2];
	float b = pos->f[0] - obj->prop->pos.f[0];
	float c = pos->f[2] - obj->prop->pos.f[2];
	float d = -b * dir->f[2] + c * dir->f[0];
...
bool objDrop(struct prop *prop, bool lazy)
{
	...
	if (parent && (obj->hidden & OBJHFLAG_PROJECTILE)) {
		...
		if (projectile->droptype == DROPTYPE_5) {
			...
			propUpdatePositionRoomsSimple(&root->pos, root->rooms, &spe4, rooms);
		} else {
			if (projectile->droptype == DROPTYPE_SURRENDER && parent->type == PROPTYPE_CHR) {
				...
				float x = parent->pos.x - playerprop->pos.x;
				float z = parent->pos.z - playerprop->pos.z;
				...
				// DROPTYPE_OWNERREAP
				// This gives dropped weapons a random velocity instead of just dropping straight down.
				mtxRandomToss(&projectile->speed, (Mtx*)&projectile->mtx);
			}

			if (!lazy && (prop->flags & PROPFLAG_ONTHISSCREENTHISTICK)) {
				// Do collision checks
				Mtx *sp48 = modelGetRootMtx(model);
				mtxApplyAffineTransform(camGetProjectionMtx(), sp48, &spf0);
				...
				// No collision checks
				mtxIdent(&spf0);
				mtxScaleRotationPart(model->scale, &spf0);
 ...
 */
void objDestroySupportedObjects(struct prop *tableprop, int playernum)
{
	struct prop *prop;
	int16_t *propnumptr;
	int16_t propnums[256];
...
void objCheckDestroyed(struct defaultobj *obj, struct coord *pos, int playernum)
{
	if (obj->damage > obj->maxdamage || objGetDestroyedLevel(obj)) {
		...
		int16_t exptype = g_PropExplosionTypes[8 + obj->modelnum];
		RoomNum rooms[8];
		...
		if (objGetDestroyedLevel(obj) == 0) {
			...
			propUpdatePositionRoomsSimple(&rootprop->pos, rootprop->rooms, pos, rooms);
			...
			int shotstaken = objGetShotsTaken(obj);
...
bool objTestAABBIntersection(struct model *model, struct modelnode *node, struct coord *arg2, struct coord *arg3, struct hitthing *hitthing, int *mtxindexptr, struct modelnode **nodeptr)
{
	int i;
	int mtxindex;
	bool ok;
	...
	mtxInvertAffine(&model->matrices[mtxindex], &mtx);
	...
	if (g_ExtraBoundsDist != 0.0f) {
		min.x = rodata->xmin - g_ExtraBoundsDist;
		min.y = rodata->ymin - g_ExtraBoundsDist;
		min.z = rodata->zmin - g_ExtraBoundsDist;

		max.x = rodata->xmax + g_ExtraBoundsDist;
		max.y = rodata->ymax + g_ExtraBoundsDist;
		max.z = rodata->zmax + g_ExtraBoundsDist;
	} else {
		min.x = rodata->xmin;
		min.y = rodata->ymin;
		min.z = rodata->zmin;
...
void doorDestroyGlass(struct doorobj *door)
{
	...
	shardsCreate((struct coord *) &matrix[3][0], &matrix[0][0], &matrix[1][0], &matrix[2][0],
...
bool objIsMortal(struct defaultobj *obj)
{
	...
	if (objIsMovablePickup(obj) && obj->type != OBJTYPE_SHIELD) {
...
void objDamage(struct defaultobj *obj, float damage, struct coord *pos, int weaponnum, int playernum)
{
	...
	if (objGetDestroyedLevel(obj) == 0) {
		...
		float max = 4 - (objGetShotsTaken(obj) % 4);
	...
	if (obj->modelnum == MODEL_TARGET) {
		...
		if (obj->type == OBJTYPE_MULTIAMMOCRATE) {
			if (objGetDestroyedLevel(obj) == 1) {
				...
				int startindex = rngRandom() % ARRAYCOUNT(crate->slots);
				int i = startindex;
				...
				do {
					if (crate->slots[i].quantity > 0 && crate->slots[i].modelnum != 0xffff) {
						...
						if (newcrate) {
							int modelnum = crate->slots[i].modelnum;
			...
			if (objGetDestroyedLevel(obj) == 0) {
				shield->amount = shield->initialamount * (float)(obj->maxdamage - obj->damage) / (float)obj->maxdamage;
			} else {
				shield->amount = 0;
			}
		}
...
void propTestModelHit(struct prop *prop, struct shotdata *shotdata)
{
	...
	if (g_ExtraBoundsDist > 0.0f) {
		...
		while (hitpart > 0) {
			if (objTestAABBIntersection(model, node1, &shotdata->gunpos2d, &shotdata->gundir2d, &hitthing1, &spe4, &node2)) {
				break;
			}
	...
	if (obj->flags3 & OBJFLAG3_HOVERBEDSHIELD) {
		...
		if (node3 && objTestAABBIntersection(model, node3, &shotdata->gunpos2d, &shotdata->gundir2d, &hitthing2, &sp90, &node4)) {
			if (hitpart <= 0 ||
					model->matrices[sp90][0][2] * hitthing2.pos.f[0] + model->matrices[sp90][1][2] * hitthing2.pos.f[1] + model->matrices[sp90][2][2] * hitthing2.pos.f[2] >
					model->matrices[spe4][0][2] * hitthing1.pos.f[0] + model->matrices[spe4][1][2] * hitthing1.pos.f[1] + model->matrices[spe4][2][2] * hitthing1.pos.f[2]) {
	...
	if (hitpart > 0) {
		mtx4TransformVec((Mtx*)&model->matrices[spe4], &hitthing1.pos, &spd8);
		spd4 = -spd8.f[2];
		...
		if (spd4 <= shotdata->distance) {
			...
			mtx4TransformVec(camGetProjectionMtx(), &spd8, &sp7c);
			mtx4RotateVec((Mtx*)&model->matrices[spe4], &hitthing1.unk0c, &sp70);
			mtx4RotateVecInPlace(camGetProjectionMtx(), &sp70);
...
void objTestHit(struct prop *prop, struct shotdata *shotdata)
{
	...
	if ((prop->flags & PROPFLAG_ONTHISSCREENTHISTICK)
		...
		tmp = -(model->matrices[0][3][2] + objGetRotatedLocalZMaxByMtx4(bbox, (Mtx*)model->matrices));
...
void objHit(struct shotdata *shotdata, struct hit *hit)
{
	...
	mtx4TransformVecInPlace(camGetProjectionMtx(), &sp110);
	...
	if (obj->modelnum == MODEL_TARGET) {
		if (hit->hitthing.texturenum == TEXTURE_0B9E) {
			...
			spe4 = hit->prop;
			mtx4TransformVec((Mtx*)obj->model->matrices, &sp110, &spec);
			tmp = -spec.z;
	...
	if (!ismeleefunc
		...
		if (!hit->slowsbullet) {
			...
			int8_t iswindoweddoor = obj->model->definition->skel == &g_SkelWindowedDoor ? true : false;
	...
	if (g_Vars.antiplayernum < 0 || g_Vars.currentplayer != g_Vars.anti || (obj->flags2 & OBJFLAG2_IMMUNETOANTI) == 0) {
		if (hit->hitthing.texturenum != 10000) {
			float damage = gsetGetDamage(&shotdata->gset);
		...
		if ((obj->hidden & OBJHFLAG_MOUNTED) == 0 && (obj->hidden & OBJHFLAG_GRABBED) == 0) {
			if (obj->flags3 & OBJFLAG3_PUSHABLE) {
				...
				mtx4MultMtx4(camGetProjectionMtx(), (Mtx*)&obj->model->matrices[hit->mtxindex], &sp58);
...
uint32_t propobjGetCiTagId(struct prop *prop)
{
	if (prop && g_Vars.stagenum == STAGE_CITRAINING) {
		uint8_t tags[8] = { 0x0e, 0x0f, 0x10, 0x47, 0x46, 0x45, 0x1b, 0x7f };
		struct defaultobj *obj = prop->obj;
		uint32_t i;
...
bool objTestForInteract(struct prop *prop)
{
	...
	if (maybe
		...
		struct prop *playerprop = g_Vars.currentplayer->prop;
		float x = prop->pos.x - playerprop->pos.x;
		float y = prop->pos.y - playerprop->pos.y;
		float z = prop->pos.z - playerprop->pos.z;
		float range;
		...
		if (x * x + z * z < range * range && y < range && y > -range) {
			float angle = atan2f(x, z) - (360.0f - g_Vars.currentplayer->vv_theta) * M_TAU / 360.0f;
...
bool propobjInteract(struct prop *prop)
{
	struct defaultobj *obj = prop->obj;
	bool result = false;
	uint32_t tag_id = propobjGetCiTagId(prop);
		...
		if (obj->type == OBJTYPE_AUTOGUN) {
			...
			if (playernum >= 0 && laptop == &g_ThrownLaptops[playernum]) {
				...
				if (laptop->ammoquantity > 0 && laptop->ammoquantity != 255) {
					int newqty = bgunGetAmmoQtyForWeapon(WEAPON_LAPTOPGUN, FUNC_PRIMARY) + laptop->ammoquantity;
...
bool objUpdateGeometry(struct prop *prop, uint8_t **start, uint8_t **end)
{
	...
	if (obj->unkgeo && (obj->flags3 & OBJFLAG3_WALKTHROUGH) == 0) {
		if (obj->hidden2 & OBJH2FLAG_DYNAMIC_COLLISION) {
			int len = (obj->flags3 & OBJFLAG3_GEOCYL) ? sizeof(struct geocyl) : sizeof(struct geoblock);
...
void objGetBbox(struct prop *prop, float *radius, float *ymax, float *ymin)
{
	...
	if (obj->unkgeo && obj->hidden2 & OBJH2FLAG_DYNAMIC_COLLISION) {
		if (obj->flags3 & OBJFLAG3_GEOCYL) {
...
struct nameinfo {
	int id;
	uint16_t singulartext;
	uint16_t pluraltext;
	uint8_t flags[5];
};

struct nameinfo *func0f087888pf(int id, struct nameinfo *info)
{
	if (info) {
		while (info->id) {
			if (info->id == id) {
...
void ammotypeGetPickupName(char *dst, int ammotype2, int qty)
{
	int ammotype = ammotype2;
...
void weaponGetPickupText(char *buffer, int weaponnum, bool dual)
{
	...
	if (dual) {
		strcat(buffer, langRemoveNewline(langGet(L_PROPOBJ_001))); // "Double"
	} else {
		if (full) {
			strcat(buffer, langRemoveNewline(langGet(L_PROPOBJ_000))); // "Picked up"
			...
			strcat(buffer, langRemoveNewline(langGet(textid)));
		} else {
			if (weaponnum == WEAPON_EYESPY && g_Vars.currentplayer->eyespy) {
	...
	if (full) {
		strcat(buffer, langRemoveNewline(bgunGetName(weaponnum)));
		plural = weaponHasFlag(weaponnum, WEAPONFLAG_DETERMINER_F_SOME);
	} else {
		strcat(buffer, langRemoveNewline(bgunGetShortName(weaponnum)));
	...
	case OBJTYPE_AMMOCRATE:
		{
			...
			int quantity = ammocrateGetPickupAmmoQty(crate);
	...
	case OBJTYPE_MULTIAMMOCRATE:
		{
			...
			for (i = 0; i != 19; i++) {
				int qty = crate->slots[i].quantity;
	...
	case OBJTYPE_WEAPON:
		{
			...
			if (ammotype) {
				int pickupqty = weaponGetPickupAmmoQty(weapon);

				if (pickupqty > 0) {
					int heldqty = bgunGetReservedAmmoCount(ammotype);
			...
			if (weapon->weaponnum == WEAPON_SUPERDRAGON) {
				...
				if (bgunGetReservedAmmoCount(AMMOTYPE_DEVASTATOR) < bgunGetCapacityByAmmotype(AMMOTYPE_DEVASTATOR)) {
					int quantity = bgunGetReservedAmmoCount(AMMOTYPE_DEVASTATOR) + 5;
...
int objTestForPickup(struct prop *prop)
{
	...
	struct prop *playerprop = g_Vars.currentplayer->prop;
	float xdiff = prop->pos.x - playerprop->pos.x;
	float ydiff = prop->pos.y - playerprop->pos.y;
	float zdiff = prop->pos.z - playerprop->pos.z;
	float range;
	bool usebigrange;
	bool pickup;
...
bool propGetScreenBoundsIfVisible(struct prop *prop, struct coord *arg1, float *arg2, float *arg3)
{
	if (prop->flags & PROPFLAG_ONTHISSCREENTHISTICK) {
		struct defaultobj *obj = prop->obj;
		Mtx *matrix = modelGetRootMtx(obj->model);
...
{
	int handnum = (weapon->base.flags & OBJFLAG_WEAPON_LEFTHANDED) ? HAND_LEFT : HAND_RIGHT;

	if (weapon->base.prop && weapon->base.model) {
		if (g_Vars.mplayerisrunning) {
			int playernum = mpPlayerGetIndex(chr);
...
struct prop *func0f08b108(struct weaponobj *weapon, struct chrdata *chr, struct modeldef *modeldef, struct prop *prop, struct model *model)
{
	...
	if (prop && weapon->base.model) {
		float scale = weapon->base.extrascale * (1.0f / 256.0f);
...
{
	int modelnum = weapon->base.modelnum;
	...
	if (weapon && prop && model) {
		...
		*weapon = tmp;
		...
		default:
			...
			if (g_Vars.mplayerisrunning) {
				int index = mpPlayerGetIndex(chr);
...
struct prop *chrGiveWeaponWithAutoModel(struct chrdata *chr, int weaponnum, uint32_t flags)
{
...
bool doorIsPosInRange(struct doorobj *door, struct coord *pos, float distance, bool isbike)
{
	struct coord range;
...
void doorUpdateTiles(struct doorobj *door)
{
	...
	if (door->doorflags & DOORFLAG_0080) {
		...
		if (door->doortype == DOORTYPE_AZTECCHAIR) {
			...
			if (door->base.flags & OBJFLAG_DOOR_OPENTOFRONT) {
				mtxRotate(&sp98, 360 - door->frac, pad.normal.x, pad.normal.y, pad.normal.z);
			} else {
				mtxRotate(&sp98, door->frac, pad.normal.x, pad.normal.y, pad.normal.z);
			}
		} else {
			if (door->base.flags & OBJFLAG_DOOR_OPENTOFRONT) {
		...
		door->base.prop->pos.x = spdc[3][0];
		door->base.prop->pos.y = spdc[3][1];
		door->base.prop->pos.z = spdc[3][2];
	}
...
struct prop *doorInit(struct doorobj *door, struct coord *pos, Mtx *mtx, RoomNum *rooms, struct coord *coord, struct coord *centre)
{
	...
	door->base.flags |= OBJFLAG_GEOMETRY_TEMP_ENABLED;
	...
	if (prop != NULL) {
		...
		mtx4Copy(mtx, &sp38);
		mtxScaleRotationPart(g_ModelStates[door->base.modelnum].scale * (1.0f / 4096.0f), &sp38);
		...
		propUpdatePositionRoomsSimple(pos, rooms, centre, sp28);
...
void doorStartOpen(struct doorobj *door)
{
	...
	if (door->doortype == DOORTYPE_FALLAWAY) {
		...
		if (cyl && (door->base.flags & OBJFLAG_GEOMETRY_TEMP_ENABLED)) {
...
int func0f08e5a8(RoomNum *rooms2, struct screenbox *box)
{
	bool result = false;
...
bool doorCalcIntendedFrac(struct doorobj *door)
{
	...
	if (door->doortype == DOORTYPE_LASER && door->fadetime60 != 0) {
		...
		if (door->mode == DOORMODE_OPENING) {
			door->laserfade = (uint32_t)((door->fadetime60 * 255.0f) / TICKS(60.0f));
			return false;
		}

		door->laserfade = (uint32_t)(((TICKS(60.0f) - door->fadetime60) * 255.0f) / TICKS(60.0f));
	}

	if (door->mode == DOORMODE_OPENING || door->mode == DOORMODE_CLOSING) {
		float end = door->mode == DOORMODE_OPENING ? door->maxfrac : 0;

		// Skedar Ruins random door stuckage
		if (door->base.flags3 & OBJFLAG3_DOOR_STICKY) {
			int value = (rngRandom() % 64) + 30;
...
void doorsCalcFrac(struct doorobj *door)
{
	...
	while (loopdoor) {
		*(float *)&loopdoor->lastcalc60 = loopdoor->frac;
	...
	while (loopdoor) {
		if (checkcollision) {
			if (cdresult != CDRESULT_COLLISION) {
				...
				loopdoor->fracspeed = 0;
				loopdoor->frac = *(float *)&loopdoor->lastcalc60;
...
bool doorTestForInteract(struct prop *prop)
{
	...
	if ((door->base.flags & OBJFLAG_CANNOT_ACTIVATE) == 0
		...
		float xdiff = door->startpos.x - playerprop->pos.x;
		float ydiff = door->startpos.y - playerprop->pos.y;
		float zdiff = door->startpos.z - playerprop->pos.z;
		...
		if (xdiff * xdiff + zdiff * zdiff < 40000 && ydiff < 200 && ydiff > -200) {
			...
			if (func0f06797c(&playerprop->pos, 150, door->base.pad)) {
				...
				if (utilsSphereIntersectsOrientedBbox(&playerprop->pos, 150, &bbox, &matrix)) {
					maybe = true;
				}
			}
		}
...
void doorsChooseSwingDirection(struct prop *chrprop, struct doorobj *door)
{
	if ((door->base.flags & OBJFLAG_DOOR_TWOWAY) && door->mode == DOORMODE_IDLE && door->frac == 0) {
		...
		if ((int)((door->base.flags ^ wantflag) << 2) < 0) {
...
Gfx *countdownTimerRender(Gfx *gdl)
{
	...
	if (!g_CountdownTimerOff) {
		float value60 = g_CountdownTimerValue60;
		int viewright = viGetViewLeft() + (viGetViewWidth() >> 1);
		int y = viGetViewTop() + viGetViewHeight() - 18;
		int playercount = PLAYERCOUNT();
		char *fmt = ":\n";
		...
		secs = (int)floorf(value60 * (1.0f / 60.0f)) - mins * 60;
		ms = (int)floorf(value60 * 1.6666666269302f) - mins * 6000 - secs * 100;
	...
	if (alarmIsActive()) {
		...
		if (!lvIsPaused()) {
			if (g_AlarmAudioHandle) {
				...
				float increment = g_Vars.lvupdate240 / 15.0f;
...
void projectileCreate(struct prop *fromprop, struct fireslotthing *arg1, struct coord *pos, struct coord *dir, uint8_t weaponnum, struct prop *targetprop)
{
	if (!lvIsPaused()) {
		bool blocked = false;
		struct coord endpos;
			...
			if (blocked) {
				if (obstacle) {
					if (obstacle->type == PROPTYPE_CHR || obstacle->type == PROPTYPE_PLAYER) {
						...
						propUpdatePositionRoomsSimple(pos, fromprop->rooms, &endpos, sp1c8);
