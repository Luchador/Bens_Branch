...
bool projectileFindCollidingProp(struct prop *prop, struct coord *pos1, struct coord *pos2, uint32_t cdtypes, struct coord *arg4, struct coord *arg5, RoomNum *rooms)
{
	...
	mtx4TransformVecInPlace((Mtx*)camGetWorldToScreenMtxf(), &sp88);

	sp7c.x = sp98.x;
	sp7c.y = sp98.y;
	sp7c.z = sp98.z;

	mtx4RotateVecInPlace((Mtx*)camGetWorldToScreenMtxf(), &sp7c);
									...
									if (ownershield <= 0.0f) {
										...
										if (g_EmbedProp->flags & PROPFLAG_ONTHISSCREENTHISTICK) {
											if (g_EmbedModel && g_EmbedHitPart != HITPART_GUN && g_EmbedHitPart != HITPART_HAT) {
												...
												mtx4TransformVec((Mtx*)camGetWorldToScreenMtxf(), &sp5e8, &sp1c8);
...
void doorInitMatrices(struct prop *prop)
{
	...
	mtxApplyAffineTransformInPlace((Mtx*)camGetWorldToScreenMtxf(), (Mtx*)matrices);
...
void cctvInitMatrices(struct prop *prop, Mtxf *mtx)
{
	...
	mtxApplyAffineTransformInPlace((Mtx*)camGetWorldToScreenMtxf(), (Mtx*)&matrices[1]);
}
...
void objInitMatrices(struct prop *prop)
{
	...
	if (obj->type == OBJTYPE_DOOR) {
		...
		mtxApplyAffineTransform((Mtx*)camGetWorldToScreenMtxf(), (Mtx*)&mtx, (Mtx*)obj->model->matrices);
...
int objTickPlayer(struct prop *prop)
{
	...
	if (model->anim) {
		if (g_Anims[model->anim->animnum].flags & ANIMFLAG_ABSOLUTETRANSLATION) {
			...
			mtx4MultMtx4((Mtx*)camGetWorldToScreenMtxf(), (Mtx*)&sp248, (Mtx*)&sp152);
...
Gfx *gfxRenderRadialShadow(Gfx *gdl, float x, float y, float z, float angle, float radius, uint32_t colour)
{
	...
	mtx4MultMtx4((Mtx*)camGetWorldToScreenMtxf(), (Mtx*)&spc0, (Mtx*)&sp80);
